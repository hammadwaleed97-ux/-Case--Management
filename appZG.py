...
def messaging_page():
    ...
    image_file = st.file_uploader("صورة البيان", type=["jpg","jpeg","png"], key=f"msg_image_{sr}")
...
def show_top_notices():
    ...
    card = f"<div style='background:linear-gradient(135deg,#101b31,#203b63);border:2px solid #D4AF37;border-radius:18px;padding:16px;margin:0 0 10px;box-shadow:0 10px 28px rgba(0,0,0,.3);text-align:center;'>"
    card += _photo_html(photo, photo_type, 70)
    card += f"<div style='color:#D4AF37;font-size:22px;font-weight:900;'>📢 {title}</div>"
    card += f"<div style='color:#fff;font-size:18px;font-weight:900;margin-top:5px;'>المرسل: {html.escape(str(sender_name))}</div>"
    ...
    with c1:
        if st.button("⬅️ السابقة", disabled=(idx >= total-1), use_container_width=True, key="top_notice_prev"):
            st.session_state.top_notice_index = min(total-1, idx+1)
            st.rerun()
    with c2:
        st.markdown(f"<div style='text-align:center;color:#fff;font-weight:900;padding-top:8px;'>البيان {idx+1} من {total}</div>", unsafe_allow_html=True)
    with c3:
        if st.button("التالية ➡️", disabled=(idx <= 0), use_container_width=True, key="top_notice_next"):
...
def show_member_directory():
    ...
    st.markdown("<div style='color:#D4AF37;font-size:22px;font-weight:900;text-align:center;margin:12px 0;'>العضويات والقيادات</div>", unsafe_allow_html=True)
    for area in sorted(set((u.get("insurance_area") or "غير محددة").strip() or "غير محددة" for u in users), key=lambda x:x.casefold()):
        st.markdown(f"<div style='color:#D4AF37;font-size:18px;font-weight:900;margin:12px 0 8px;'>المنطقة: {html.escape(area)}</div>", unsafe_allow_html=True)
        ...
        for i,u in enumerate(area_users):
            with cols[i % 4]:
                ...
                job = html.escape(str(u.get("job_title") or role_label(u.get("role"))))
                photo = _photo_html(u.get("profile_photo"), u.get("profile_photo_type") or "image/png", 58)
                status = html.escape(str(u.get("status") or ""))
                card = f"<div style='background:#142038;border:1px solid #D4AF37;border-radius:14px;padding:9px;text-align:center;min-height:160px;'>{photo}<div style='color:#fff;font-size:14px;font-weight:900;'>{name}</div><div style='color:#D4AF37;font-size:12px;font-weight:800;'>{job}</div><div style='color:#cbd5e1;font-size:11px;'>{html.escape(area)}</div><div style='color:#9fb3c8;font-size:10px;'>{status}</div></div>"
                st.markdown(card, unsafe_allow_html=True)
                if u.get("id") != current.get("id") and can_message(current.get("role"), u.get("role")):
                    if st.button("✉️ إرسال", key=f"dir_msg_{current.get('id')}_{u.get('id')}", use_container_width=True):
