                ...
                for idx, case in enumerate(results, 1):
                    ...
                    with c2:
                        if st.button(f"📂 فتح في {مكان}", key=f"open_smart_btn_{case['id']}", use_container_width=True):
                            ...
                            st.rerun()
                    st.markdown("</div>", unsafe_allow_html=True)
                    # ========== صفحة مركز التنبيهات - مع التشخيص ==========
elif st.session_state.page == "التنبيهات":
    data = load_data()
    st.markdown("<h2 style='color:#FF5252; text-align:center'>🔔 مركز التنبيهات - وضع التشخيص</h2>", unsafe_allow_html=True)

    if st.button("⬅️ العودة للرئيسية"): st.session_state.page = "الرئيسية"; st.rerun()

    today = datetime.now().date()
    st.write(f"**تاريخ النهاردة في السيستم:** {today}")
    all_cases = data["cases"]
    st.write(f"**عدد القضايا الكلي:** {len(all_cases)}")

    st.markdown("---")
    # ====== تشخيص الجلسات ======
    st.markdown("<h3 style='color:#4DA8DA'>📅 تشخيص الجلسات</h3>", unsafe_allow_html=True)
    
    alert_sessions = []
    for case in all_cases:
        st.write(f"بفحص قضية: {case['رقم']}/{case['سنة']} - الحالة: {case.get('حالة')} - تاريخ الجلسة: {case.get('تاريخ_جلسة')}")
        if case.get('حالة') == 'متداولة' and case.get('تاريخ_جلسة'):
            try:
                session_date = datetime.strptime(case['تاريخ_جلسة'], '%Y-%m-%d').date()
                days_left = (session_date - today).days
                st.write(f"  -> تاريخ الجلسة: {session_date} | متبقي: {days_left} يوم")
                if 0 <= days_left <= 7:
                    case['days_left'] = days_left
                    alert_sessions.append(case)
                    st.success(f"  -> ✅ دي هتظهر في التنبيه")
            except Exception as e: 
                st.error(f"  -> ❌ خطأ في تاريخ الجلسة: {e}")

    st.markdown("---")
    st.markdown("### النتيجة النهائية")
    if alert_sessions:
        st.success(f"تم العثور على {len(alert_sessions)} جلسة")
        for i, case in enumerate(alert_sessions):
            st.write(f"{i+1}. {case['رقم']}/{case['سنة']} - {case['مدعي']} - {case['تاريخ_جلسة']} - متبقي {case['days_left']} يوم")
    else:
