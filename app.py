...
# ==================================================================
    ...
    if not data["cases"]:
        ...
        for i, case in enumerate(data["cases"]):
            ...
            if "حالة" not in case: case["حالة"] = "متداولة"
        save_data(data)
        ...
        st.markdown("<table class='case-table'><thead><tr><th>م</th><th>رقم/سنة</th><th>المحكمة والدائرة</th><th>الخصوم</th><th>الموضوع</th><th>اخر جلسة</th><th>السبب</th><th>الحالة</th><th>عرض</th></tr></thead><tbody>", unsafe_allow_html=True)
        ...
        for idx, case in enumerate(sorted_cases, 1):
            ...
            if case.get('حالة') == 'حكم': row_class = "row-judgment"
            elif case.get('حالة') == 'هيئة': row_class = "row-hey2a"
            else: row_class = "row1" if idx % 2 == 1 else "row2"

            # زر احمر صغير
            btn = f"<form action='' method='get'><button style='background:#8B0000; color:#FFD700; border:2px solid #FFD700; padding:5px 12px; border-radius:6px; font-weight:700; cursor:pointer;'>فتح</button><input type='hidden' name='page' value='تفاصيل'><input type='hidden' name='id' value='{case['id']}'></form>"

            st.markdown(f"<tr class='{row_class}'><td>{idx}</td><td>{رقم_كامل}</td><td>{محكمة_كاملة}</td><td>{خصوم}</td><td>{case.get('موضوع','')}</td><td>{case.get('تاريخ_جلسة','')}</td><td>{case.get('سبب','')}</td><td>{case.get('حالة','متداولة')}</td><td>{btn}</td></tr>", unsafe_allow_html=True)
    ...
    if not case:
        st.error("القضية غير موجودة"); st.session_state.page = "حصر"; st.rerun()
    ...
    if case['مستندات']:
        st.markdown("<div class='card'><div class='card-title'>المستندات المرفوعة</div>", unsafe_allow_html=True)
        for مستند in case['مستندات']:
            st.markdown(f"<p>📎 {مستند['نوع']} - {مستند.get('بيان','')}</p>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
