    ...
    if not df.empty:
        ...
        if not upcoming.empty:
            for i, row in upcoming.iterrows():
                st.markdown(f"""
                <div class='card'>
                    <b>رقم:</b> {row['رقم']} لسنة {row['سنة']}<br>
                    <b>المحكمة:</b> {row['محكمة_اسم']}<br>
                    <b>التاريخ:</b> {row['تاريخ_جلسة']}<br>
                    <b>السبب:</b> {row['سبب']}
                </div>
...
.row-judgment {background:#FFDCDC; font-weight:800}
    ...
    /* زرار التنبيهات الاحمر الغامق */
    div[data-testid="stButton"] > button[key="notif_btn"] {
