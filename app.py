        ...
        if not search_value.strip():
            ...
            for case in data["cases"]:
                ...
                if search_type == "اسم المدعي او المدعى عليه":
                    ...
                    if رقم_بحث == str(case.get('رقم','')) and سنة_بحث == str(case.get('سنة','')) و "استئنافية" in نوع_الدعوى:
                        match = True
            ...
            if not results:
                ...
                for idx, case in enumerate(results, 1):
                    ...
                    # زر فتح القضية في النص زي عندك
                    c1, c2, c3 = st.columns([4,1,4])
                    with c2:
                        if st.button("📂 فتح", key=f"open_search_{case['id']}_{idx}", use_container_width=True):
                            st.session_state.selected_case_id = case['id']
                            st.session_state.page = "تفاصيل" # <-- ده المهم مطابق للي عندك
                            st.rerun()
