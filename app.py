...
def send_case_alert_email(recipient_email, case, نوع_التنبيه):
    ...
    if "verify_token" in query_params:
        ...
        if email:
            st.success(f"✅ تم تفعيل الايميل {email} بنجاح")
        ...
        if st.button("ارسال رابط التفعيل", type="primary", use_container_width=True):
            if user_email:
                ...
                if send_verification_email(user_email, token):
                    ...
                    st.error("فشل ارسال الايميل. راجع SENDER_EMAIL و SENDER_PASSWORD")
            else:
    ...
    if data["cases"]:
        ...
        if not upcoming.empty:
            for idx, row in enumerate(upcoming.iterrows(), 1):
                ...
                st.markdown(f"""
                <div style='border:2px solid #FF5252; border-radius:12px; padding:10px; margin-bottom:15px;'>
                    <table style='width:100%; font-size:14px;'>
                        <tr style='background:#FF5252; color:#FFF; font-weight:900;'>
                            <th>م</th><th>الرقم والسنة</th><th>المحكمة</th><th>الخصوم</th><th>الموضوع</th><th>الجلسة</th><th>السبب</th>
                        </tr>
                        <tr style='background:#1E2A47; color:#FFF;'>
                            <td style='text-align:center;'>{idx}</td>
                            <td style='text-align:center;'>{رقم_كامل}</td>
                            <td style='text-align:center;'>{محكمة_كاملة}</td>
                            <td style='text-align:center;'>{خصوم}</td>
                            <td>{case.get('موضوع','')}</td>
                            <td style='text-align:center;'>{case['تاريخ_جلسة']}</td>
                            <td style='text-align:center;'>{case.get('سبب','')}</td>
                        </tr>
                    </table>
                </div>
                """, unsafe_allow_html=True)

                if st.button("فتح القضية", key=f"open_notif_{case['id']}"):
                    st.session_state.selected_case_id = case['id']
                    st.session_state.page = "تفاصيل"
                    st.rerun()
        else:
    ...
    if appeals_list:
        ...
        for idx, case in enumerate(appeals_list, 1):
            ...
            st.markdown(f"""
            <div style='border:3px solid {لون}; padding:10px; border-radius:10px; margin-bottom:10px; background:#1E2A47;'>
                <h4 style='color:{لون};'> {idx}. {رقم_كامل}</h4>
                <b>الحالة:</b> {حالة} | <b>ينتهي:</b> {case['تاريخ_انتهاء_الطعن_محسوب']} | <b>المدة:</b> {case['مدة_الطعن']} يوم<br>
                <b>الحكم:</b> {case.get('الإجراءات','')}
            </div>
            """, unsafe_allow_html=True)
