...
# ================================================
    ...
    with tab1:
        ...
        if st.button("🔍 عرض بيان الدعاوى المتداولة", use_container_width=True, type="primary", key="show1"):
            ...
            if not cases: st.warning("لا توجد دعاوى متداولة في الفترة المحددة")
                ...
                st.download_button("⬇️ تحميل PDF", data=pdf_buffer, file_name=f"report_motadawla_{datetime.now().strftime('%Y%m%d')}.pdf", mime="application/pdf", use_container_width=True)

            st.markdown(f"<p style='text-align:right; color:#D4AF37; margin-top:30px; font-size:16px; font-family:Cairo'>تفضلوا بقبول وافر الاحترام<br><br>عضو الادارة.................. مدير الإدارة..................<br>تحرر في {datetime.now().strftime('%Y-%m-%d')}</p>", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if st.button("🔍 عرض بيان الاحكام", use_container_width=True, type="primary", key="show2"):
            ...
            if not cases: st.warning("لا توجد احكام في الفترة المحددة")
                ...
                st.download_button("⬇️ تحميل PDF", data=pdf_buffer, file_name=f"report_ahkam_{datetime.now().strftime('%Y%m%d')}.pdf", mime="application/pdf", use_container_width=True)
    ...
    # ========= تبويب 4: التصدير =========
    with tab4:
        st.markdown("<h3 style='color:#D4AF37; text-align:center; font-family:Cairo'>📄 تصدير التقارير</h3>", unsafe_allow_html=True)
        st.success("استخدم ازرار التحميل اللي ظهرت بعد عرض التقرير في التبويب 1 و 2")
