...
# ================================================
    ...
    def create_pdf(cases_list, filename, report_type, region, title):
        ...
        elements.append(Paragraph(arabic("الإدارة العامة للقضايا"), styleN))
        elements.append(Paragraph(arabic(f"ديوان عام {region}"), styleN))
        elements.append(Spacer(1, 12))
        elements.append(Paragraph(arabic(title), styleH))
        elements.append(Spacer(1, 24))

        if cases_list:
            if report_type == "متداولة":
                data_table = [[arabic('م'), arabic('رقم القضية'), arabic('السنة القضائية'), arabic('الدائرة والنوع'), arabic('اسم المحكمة والمأمورية'), arabic('الخصوم'), arabic('موضوع الدعوى'), arabic('اخر جلسة وسببها'), arabic('ملاحظات')]]
                for i, c in enumerate(cases_list, 1):
                    ...
                    خصوم = f"المدعى: {c.get('مدعي','')}\nالمدعى عليه: {c.get('مدعي_عليه','')}"
                    جلسة = f"{c.get('تاريخ_جلسة','')}\n{c.get('سبب','')}"
                    data_table.append([i, c.get('رقم',''), c.get('سنة',''), دائرة, محكمة, خصوم, c.get('موضوع',''), جلسة, c.get('ملاحظات','')])
            else:
                data_table = [[arabic('م'), arabic('رقم القضية'), arabic('السنة القضائية'), arabic('الدائرة والنوع'), arabic('اسم المحكمة والمأمورية'), arabic('الخصوم'), arabic('موضوع الدعوى'), arabic('تاريخ جلسة الحكم'), arabic('منطوق الحكم'), arabic('النتيجة'), arabic('ملاحظات')]]
                for i, c in enumerate(cases_list, 1):
                    محكمة = f"{c.get('محكمة_اسم','')}"
                    if c.get('مأمورية'): محكمة += f" - مأمورية {c.get('مأمورية')}"
                    دائرة = f"{c.get('دائرة','')} {c.get('نوع','')}"
                    خصوم = f"المدعى: {c.get('مدعي','')}\nالمدعى عليه: {c.get('مدعي_عليه','')}"
                    data_table.append([i, c.get('رقم',''), c.get('سنة',''), دائرة, محكمة, خصوم, c.get('موضوع',''), c.get('تاريخ_الحكم',''), c.get('منطوق_الحكم',''), c.get('مسندة_ل_الحكم',''), c.get('ملاحظات','')])

            table_data = [[Paragraph(arabic(str(cell)), styleN) for cell in row] for row in data_table]

            t = Table(table_data, repeatRows=1)
            t.setStyle(TableStyle([
                ('BACKGROUND', (0,0), (-1,0), colors.HexColor('#D4AF37')), # دهبي للهيدر
                ('TEXTCOLOR',(0,0),(-1,0), colors.black),
                ('ALIGN',(0,0),(-1,-1),'CENTER'),
                ('FONTNAME', (0,0), (-1,-1), 'Cairo'),
                ('FONTSIZE', (0,0), (-1,0), 11),
                ('BOTTOMPADDING', (0,0), (-1,0), 12),
                ('BACKGROUND',(0,1),(-1,-1), colors.HexColor('#0A1428')), # كحلي غامق
                ('GRID', (0,0), (-1,-1), 1, colors.HexColor('#D4AF37')),
                ('ROWBACKGROUNDS', (0,1), (-1,-1), [colors.HexColor('#1E2A47'), colors.HexColor('#0A1428')]) # تبادل الوان الصفوف
            ]))
            elements.append(t)

        elements.append(Spacer(1, 24))
        elements.append(Paragraph(arabic(f"تفضلوا بقبول وافر الاحترام"), styleN))
        elements.append(Paragraph(arabic(f"عضو الادارة.................. مدير الإدارة.................."), styleN))
        elements.append(Paragraph(arabic(f"تحرر في {datetime.now().strftime('%Y-%m-%d')}"), styleN))
    ...
    def create_word(cases_list, filename, report_type, region, title):
        doc = Document()
        p = doc.add_paragraph(); p.alignment = WD_ALIGN_PARAGRAPH.CENTER; run = p.add_run('الهيئة القومية للتأمين الاجتماعى'); run.font.name = 'Cairo'; run.font.size = Pt(16); run.bold = True; run.font.color.rgb = RGBColor(212, 175, 55)

        for txt in ['الإدارة المركزية للإدارات القانونية', 'الإدارة العامة للقضايا', f'ديوان عام {region}', title]:
            p = doc.add_paragraph(txt); p.alignment = WD_ALIGN_PARAGRAPH.CENTER; p.runs[0].font.name = 'Cairo'
        ...
        if cases_list:
            if report_type == "متداولة":
                ...
                headers = ['م','رقم القضية','السنة القضائية','الدائرة والنوع','اسم المحكمة والمأمورية','الخصوم','موضوع الدعوى','اخر جلسة وسببها','ملاحظات']
                for i, h in enumerate(headers):
                    hdr[i].text = h
                    for run in hdr[i].paragraphs[0].runs: run.font.bold = True; run.font.name = 'Cairo'
                    hdr[i]._tc.get_or_add_tcPr().append(doc.styles['Table Grid'].element)
            else:
                table = doc.add_table(rows=1, cols=11)
                table.style = 'Table Grid'
                hdr = table.rows[0].cells
                headers = ['م','رقم القضية','السنة القضائية','الدائرة والنوع','اسم المحكمة والمأمورية','الخصوم','موضوع الدعوى','تاريخ جلسة الحكم','منطوق الحكم','النتيجة','ملاحظات']
                for i, h in enumerate(headers):
                    hdr[i].text = h
                    for run in hdr[i].paragraphs[0].runs: run.font.bold = True; run.font.name = 'Cairo'
            ...
            for i, c in enumerate(cases_list, 1):
                ...
                if c.get('مأمورية'): محكمة += f" - مأمورية {c.get('مأمورية')}"
                if report_type == "متداولة":
                    row[0].text, row[1].text, row[2].text, row[3].text, row[4].text, row[5].text, row[6].text, row[7].text, row[8].text = str(i), c.get('رقم',''), c.get('سنة',''), f"{c.get('دائرة','')} {c.get('نوع','')}", محكمة, f"{c.get('مدعي','')}\nvs\n{c.get('مدعي_عليه','')}", c.get('موضوع',''), f"{c.get('تاريخ_جلسة','')}\n{c.get('سبب','')}", c.get('ملاحظات','')
                else:
                    row[0].text, row[1].text, row[2].text, row[3].text, row[4].text, row[5].text, row[6].text, row[7].text, row[8].text, row[9].text, row[10].text = str(i), c.get('رقم',''), c.get('سنة',''), f"{c.get('دائرة','')} {c.get('نوع','')}", محكمة, f"{c.get('مدعي','')}\nvs\n{c.get('مدعي_عليه','')}", c.get('موضوع',''), c.get('تاريخ_الحكم',''), c.get('منطوق_الحكم',''), c.get('مسندة_ل_الحكم',''), c.get('ملاحظات','')
        ...
        if cases_list:
            if report_type == "متداولة":
                ...
                for i, c in enumerate(cases_list, 1):
                    ...
                    if c.get('مأمورية'): محكمة += f" - مأمورية {c.get('مأمورية')}"
                    دائرة = f"{c.get('دائرة','')} {c.get('نوع','')}"
                    خصوم = f"المدعى: {c.get('مدعي','')} | المدعى عليه: {c.get('مدعي_عليه','')}"
                    جلسة = f"{c.get('تاريخ_جلسة','')} - {c.get('سبب','')}"
                    rows.append([i, c.get('رقم',''), c.get('سنة',''), دائرة, محكمة, خصوم, c.get('موضوع',''), جلسة, c.get('ملاحظات','')])
                df_export = pd.DataFrame(rows, columns=['م', 'رقم القضية', 'السنة القضائية', 'الدائرة والنوع', 'اسم المحكمة والمأمورية', 'الخصوم', 'موضوع الدعوى', 'اخر جلسة وسببها', 'ملاحظات'])
            else:
                rows = []
                for i, c in enumerate(cases_list, 1):
                    محكمة = f"{c.get('محكمة_اسم','')}"
                    ...
                    دائرة = f"{c.get('دائرة','')} {c.get('نوع','')}"
                    خصوم = f"المدعى: {c.get('مدعي','')} | المدعى عليه: {c.get('مدعي_عليه','')}"
                    rows.append([i, c.get('رقم',''), c.get('سنة',''), دائرة, محكمة, خصوم, c.get('موضوع',''), c.get('تاريخ_الحكم',''), c.get('منطوق_الحكم',''), c.get('مسندة_ل_الحكم',''), c.get('ملاحظات','')])
                df_export = pd.DataFrame(rows, columns=['م', 'رقم القضية', 'السنة القضائية', 'الدائرة والنوع', 'اسم المحكمة والمأمورية', 'الخصوم', 'موضوع الدعوى', 'تاريخ جلسة الحكم', 'منطوق الحكم', 'النتيجة', 'ملاحظات'])
        else:
            df_export = pd.DataFrame()

        excel_data = df_export.to_excel(index=False) if not df_export.empty else b""
        pdf_buffer = create_pdf(cases_list, filename, report_type, region, title)
        word_buffer = create_word(cases_list, filename, report_type, region, title)

        c1,c2,c3,c4,c5 = st.columns(5)
        with c1: st.download_button("📄 فتح PDF", data=pdf_buffer, file_name=f"{filename}.pdf", mime="application/pdf", use_container_width=True, key=f"open_pdf_{filename}")
        with c2: st.download_button("📝 فتح Word", data=word_buffer, file_name=f"{filename}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True, key=f"open_word_{filename}")
        with c3: st.download_button("⬇️ تحميل PDF", data=pdf_buffer, file_name=f"{filename}.pdf", mime="application/pdf", use_container_width=True, key=f"dl_pdf_{filename}")
        with c4: st.download_button("⬇️ تحميل Word", data=word_buffer, file_name=f"{filename}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True, key=f"dl_word_{filename}")
        with c5:
            st.download_button("📊 تحميل Excel", data=excel_data, file_name=f"{filename}.xlsx", use_container_width=True, key=f"dl_excel_{filename}", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    ...
    with tab1:
        ...
        if st.button("🔍 عرض بيان الدعاوى المتداولة", use_container_width=True, type="primary", key="show1"):
            ...
            if not cases:
                st.warning("لا توجد دعاوى متداولة في الفترة المحددة")
            else:
    ...
    with tab2:
        ...
        if st.button("🔍 عرض بيان الاحكام", use_container_width=True, type="primary", key="show2"):
            ...
            if topic2:
                cases = [c for c in cases if topic2 in str(c.get('موضوع',''))]
            ...
            report_header(region2, title)
            if not cases:
                st.warning("لا توجد احكام في الفترة المحددة")
            else:
    ...
    # ========= تبويب 3: الاحصائيات =========
    with tab3:
        st.markdown("<h3 style='color:#D4AF37; text-align:center'>📊 الإحصائيات العددية</h3>", unsafe_allow_
