...
# ==================== بداية قسم 4: تفاصيل القضية v5.19 FINAL ====================
elif st.session_state.page == "تفاصيل":
    case = next((c for c in data["cases"] if c['id'] == st.session_state.selected_case_id), None)
    if not case:
        st.error("القضية غير موجودة")
    ...
    with col_main:
        ...
        if case.get("جلسات"):
            ...
            <thead><tr style='background:#C9A961; color:#0F1C2E; font-weight:900; font-size:16px'>
        ...
        with st.expander("➕ اضافة جلسة"):
            with st.form("add_session"):
                ...
                if st.form_submit_button("حفظ الجلسة"):
                    جلسه_جديده = {"تاريخ": str(تاريخ_جديد), "الرول": رول_جديد, "سبب": سبب_جديد, "ملاحظات": ملاحظات_جديد}
                    if "جلسات" not in case: case["جلسات"] = []
                ...
                if جلسه_مختارة:
                    with st.form("edit_session"):
                        ...
                        if st.form_submit_button("حفظ التعديل"):
                            جلسه_مختارة.update({"تاريخ": str(تاريخ_معدل), "الرول": رول_معدل, "سبب": سبب_معدل, "ملاحظات": ملاحظات_معدل})
        ...
        with st.form("upload_doc"):
            نوع_مستند = st.selectbox("نوع المستند", ["صحيفة دعوى", "صحيفة استئناف", "صحيفة طعن", "مذكرة دفاع", "حافظة مستندات", "تقرير خبير", "تقرير طب شرعي", "تقرير لجنة طبية", "صحيفة تجديد من الشطب", "صحيفة تعجيل من الوقف", "صورة حكم تمهيدي", "اخرى"])
        ...
        with st.form("judgement_form"):
            ...
            if st.form_submit_button("حفظ الحكم"):
                case["تاريخ_الحكم"] = str(تاريخ_الحكم); case["منطوق_الحكم"] = منطوق_الحكم; case["مسند_الحكم"] = مسند
                if "جلسات" not in case: case["جلسات"] = []
                case["جلسات"].append({"تاريخ": str(تاريخ_الحكم), "الرول": "الحكم", "سبب": منطوق_الحكم, "ملاحظات": مسند})
