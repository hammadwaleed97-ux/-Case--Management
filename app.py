...
<style>
    ...
    div[data-testid="stButton"] > button {{
        background: linear-gradient(135deg, #2C4A73, #3A5F8A) !important;
        color: #E8E8E8 !important; width: 100%; padding: 20px 10px; 
        border-radius: 12px; border: 2px solid #C9A961; font-weight: 700; font-size: 15px;
        height: 90px; margin: 5px 0;
    }}
...
# ========== 1. تسجيل القضايا كامل ==========
    ...
    with st.form("form_case"):
        ...
        with col2:
            if st.form_submit_button("🗑️ حذف القضية"): st.warning("سيتم الحذف")

# ========== 2. الحصر العام بجدول الجلسات ==========
elif st.session_state.page == "حصر":
    st.markdown("<h2 style='color:#C9A961'>📊 الحصر العام</h2>", unsafe_allow_html=True)
    if st.button("⬅️ العودة للرئيسية"): st.session_state.page = "الرئيسية"
    ...
    if not data["cases"]:
        ...
        for case in sorted(data["cases"], key=lambda x: x["id"]):
            ...
            """, unsafe_allow_html=True)
            if st.button("فتح القضية ومتابعة الجلسات", key=f"open{case['id']}"):
...
# ========== 3. صفحة تفاصيل القضية ==========
    ...
    st.markdown(f"<h2 style='color:#C9A961'>📄 القضية رقم {case['رقم']} / {case['سنة']}</h2>", unsafe_allow_html=True)
    if st.button("⬅️ العودة للحصر"): st.session_state.page = "حصر"
    
    st.markdown("<h4 style='color:#C9A961'>بيانات القضية</h4>", unsafe_allow_html=True)
    st.json(case)
...
# باقي الصفحات
    ...
    st.markdown("<h2 style='color:#C9A961'>📈 الإحصائيات</h2>", unsafe_allow_html=True)
    if st.button("⬅️ العودة للرئيسية"): st.session_state.page = "الرئيسية"
