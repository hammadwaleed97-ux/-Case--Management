    ...
    with tab1:
        ...
        if st.button("🔍 عرض بيان الدعاوى المتداولة", use_container_width=True, type="primary", key="show1"):
            ...
            if not cases: st.warning("لا توجد دعاوى متداولة في الفترة المحددة")
                ...
                # نعمل df مرتب للتصدير
                export_data = []
                for i, c in enumerate(cases, 1):
                    export_data.append({
                        "م": i, "رقم القضية": c.get('رقم',''), "السنة": c.get('سنة',''),
                        "الدائرة": c.get('دائرة',''), "النوع": c.get('نوع',''), "المحكمة": c.get('محكمة_اسم',''),
                        "المأمورية": c.get('مأمورية',''), "المدعي": c.get('مدعي',''), "المدعي عليه": c.get('مدعي_عليه',''),
                        "الموضوع": c.get('موضوع',''), "تاريخ الجلسة": c.get('تاريخ_جلسة',''), "السبب": c.get('سبب',''), "ملاحظات": c.get('ملاحظات','')
                    })
                df_export = pd.DataFrame(export_data)

                html = f"<div dir='rtl' style='font-family:Cairo'>"
                ...
                with c2:
                    st.download_button("📄 تحميل Word", data=to_word(export_data, title, region), file_name=f"بيان_المتداولة_{datetime.now().strftime('%Y%m%d')}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True)
                with c3:
                    st.download_button("🖨️ تحميل HTML للطباعة", data=html.encode('utf-8'), file_name=f"بيان_المتداولة_{datetime.now().strftime('%Y%m%d')}.html", mime="text/html", use_container_width=True)
            ...
            if not cases: st.warning("لا توجد احكام في الفترة المحددة")
                ...
                for i, c in enumerate(cases, 1):
                    export_data.append({
                        ...
                        "الدائرة": c.get('دائرة',''), "النوع": c.get('نوع',''), "المحكمة": c.get('محكمة_اسم',''),
                        "المأمورية": c.get('مأمورية',''), "المدعي": c.get('مدعي',''), "المدعي عليه": c.get('مدعي_عليه',''),
                        "الموضوع": c.get('موضوع',''), "تاريخ الحكم": c.get('تاريخ_الحكم',''), "منطوق الحكم": c.get('منطوق_الحكم',''), "النتيجة": c.get('مسندة_ل_الحكم',''), "ملاحظات": c.get('ملاحظات','')
                    })
                df_export = pd.DataFrame(export_data)
                ...
                with c2:
                    st.download_button("📄 تحميل Word", data=to_word(export_data, title, region2), file_name=f"بيان_الاحكام_{datetime.now().strftime('%Y%m%d')}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True)
                with c3:
                    st.download_button("🖨️ تحميل HTML للطباعة", data=html.encode('utf-8'), file_name=f"بيان_الاحكام_{datetime.now().strftime('%Y%m%d')}.html", mime="text/html", use_container_width=True)
