...
# ================================================
    ...
    def export_buttons(df):
        ...
        with col1: st.button("📄 فتح PDF")
        with col2: st.button("📝 فتح Word") 
        with col3: st.button("⬇️ تحميل PDF")
        with col4: st.button("⬇️ تحميل Word")
        with col5: st.download_button("⬇️ تحميل Excel", df.to_csv(index=False).encode('utf-8-sig'), "تقرير.csv", "text/csv")
    ...
    with tab1:
        region = report_header()
        col1, col2, col3 = st.columns(3)
        with col1: date_from = st.date_input("من تاريخ", key="d_from1")
        with col2: date_to = st.date_input("حتى تاريخ", key="d_to1") 
        with col3: lawyer_name = st.text_input("طرف الاستاذ/ المحامى", key="law1")
        
        st.markdown(f"**بيان بالدعاوى المتداولة خلال الفترة من {date_from} حتى {date_to} طرف الاستاذ/ {lawyer_name}**")
        ...
        filtered = sorted(filtered, key=lambda x: x.get("last_session_date",""), reverse=True)
        
        df = pd.DataFrame([{
            "م": i+1, "رقم القضية": c.get("case_no"), "السنة القضائية": c.get("year"),
            "الدائرة والنوع": c.get("court_type"), "اسم المحكمة والمامورية": c.get("court"),
            "الخصوم": c.get("opponents"), "موضوع الدعوى": c.get("topic"),
            "اخر جلسة وسببها": f"{c.get('last_session_date')} - {c.get('last_session_reason')}",
            "ملاحظات": c.get("notes")
    ...
    with tab2:
        ...
        with col3: lawyer_name = st.text_input("طرف الاستاذ/ المحامى", key="law2")
        with col4: judgment_type = st.selectbox("نوع الاحكام", ["جميع الاحكام", "الاحكام للصالح", "الاحكام للضد"])
        
        st.markdown(f"**بيان بالاحكام {judgment_type} خلال الفترة من {date_from} حتى {date_to}**")
        ...
        for c in archive:
            try:
                ...
                if date_from <= d <= date_to:
                    if judgment_type == "الاحكام للصالح" and c.get("result") != "للصالح": continue
                    if judgment_type == "الاحكام للضد" and c.get("result") != "للضد": continue
                    filtered.append(c)
            except: pass
        
        filtered = sorted(filtered, key=lambda x: x.get("judgment_date",""), reverse=True)

        df = pd.DataFrame([{
            "م": i+1, "رقم القضية": c.get("case_no"), "السنة القضائية": c.get("year"),
            "الدائرة والنوع": c.get("court_type"), "اسم المحكمة والمامورية": c.get("court"),
            "الخصوم": c.get("opponents"), "موضوع الدعوى": c.get("topic"),
            "تاريخ جلسة الحكم": c.get("judgment_date"), "منطوقه": c.get("judgment_text"),
            "النتيجة": c.get("result"), "ملاحظات": c.get("notes")
        } for i,c in enumerate(filtered)])
    ...
    # ========= TAB 3: حسب الموضوع =========
    with tab3:
        report_subtype = st.selectbox("اختر نوع التقرير", ["الدعاوى المتداولة حسب الموضوع", "الاحكام الصادرة حسب الموضوع"])
        topic = st.text_input("اكتب موضوع الدعوى")
        region = report_header()
        col1, col2, col3 = st.columns(3)
        with col1: date_from = st.date_input("من تاريخ", key="d_from3")
        with col2: date_to = st.date_input("حتى تاريخ", key="d_to3")
        with col3: lawyer_name = st.text_input("طرف الاستاذ/ المحامى", key="law3")

        source = cases if "الدعاوى" in report_subtype else archive
        filtered = [c for c in source if topic.lower() in c.get("topic","").lower()]
    ...
    with tab4:
        ...
        date_from = st.date_input("من تاريخ", key="d_from4")
        date_to = st.date_input("حتى تاريخ", key="d_to4")
        
        total_cases = len([c for c in cases if c.get("status") != "منتهية"])
        total_judgments = len(archive)
        win = len([c for c in archive if c.get("result") == "للصالح"])
        lose = len([c for c in archive if c.get("result") == "للضد"])
        
        col1, col2, col3, col4 = st.columns(4)
        col1.metric("عدد القضايا المتداولة", total_cases, border=True)
        col2.metric("عدد الاحكام الصادرة", total_judgments, border=True)
        col3.metric("عدد الاحكام للصالح", win, border=True)
        col4.metric("عدد الاحكام للضد", lose, border=True)
