...
# ==================== بداية قسم 4: تفاصيل القضية v5.17 ====================
elif st.session_state.page == "تفاصيل":
    case = next((c for c in data["cases"] if c['id'] == st.session_state.selected_case_id), None)
    if not case:
        st.error("القضية غير موجودة")
    ...
    with col_main:
        ...
        if case.get("جلسات"):
            جلسات_مرتبة = sorted(case["جلسات"], key=lambda x: x['تاريخ'])
            table_html = """
            <table style='width:100%; border:3px solid #C9A961; border-radius:10px; background:#0A1428; color:#FFFFFF; border-collapse: collapse; margin-bottom:20px'>
            <tr style='background:linear-gradient(90deg, #C9A961, #D4B96A); color:#0F1C2E; font-weight:900; font-size:16px'>
                ...
                <th style='padding:12px; border:2px solid #C9A961'>التاريخ</th>
                <th style='padding:12px; border:2px solid #C9A961'>الاجراءات</th>
            </tr>"""
            for i, ج in enumerate(جلسات_مرتبة, 1):
                table_html += f"""
                <tr style='background:#1E2A47'>
                    <td style='text-align:center; padding:10px; border:2px solid #C9A961; color:#FFFFFF; font-weight:700; font-size:15px'>{i}</td>
                    <td style='text-align:center; padding:10px; border:2px solid #C9A961; color:#FFFFFF; font-weight:700; font-size:15px'>{ج.get('الرول','')}</td>
                    <td style='text-align:center; padding:10px; border:2px solid #C9A961; color:#FFFFFF; font-weight:700; font-size:15px'>{ج['تاريخ']}</td>
                    <td style='text-align:center; padding:10px; border:2px solid #C9A961; color:#FFFFFF; font-weight:700; font-size:15px'>{ج.get('سبب','')}</td>
                </tr>"""
            table_html += "</table>"
            st.markdown(table_html, unsafe_allow_html=True)
        else:
        ...
        with st.expander("➕ اضافة جلسة جديدة"):
            with st.form("add_session"):
                ...
                اجراء_جديد = c3.text_input("الاجراءات")
                if st.form_submit_button("حفظ الجلسة"):
                    جلسه_جديده = {"تاريخ": str(تاريخ_جديد), "الرول": رول_جديد, "سبب": اجراء_جديد}
                    if "جلسات" not in case: case["جلسات"] = []
                    case["جلسات"].append(جلسه_جديده)
                    # تحديث اخر جلسة في الحصر العام
                    case["تاريخ_جلسة"] = str(تاريخ_جديد); case["سبب"] = اجراء_جديد
                    save_data(data); st.success("✅ تم حفظ الجلسة"); st.rerun()
        ...
        with st.form("judgement_form"):
            ...
            if st.form_submit_button("حفظ الحكم"):
                ...
                if "جلسات" not in case: case["جلسات"] = []
                case["جلسات"].append({"تاريخ": str(تاريخ_الحكم), "الرول": "الحكم", "سبب": منطوق_الحكم})
