...
# ============================================
# ============================================
# الجزء 2: الواجهات والصفحات الرئيسية
# ============================================
def render_notification_center():
    ...
    if "verify_token" in query_params: email = verify_token(query_params["verify_token"]);
    if email: st.success(f"✅ تم تفعيل الايميل {email} بنجاح. ستصلك التنبيهات الان"); st.session_state['saved_email'] = email
    else: st.error("❌ الرابط غير صالح او منتهي"); st.query_params.clear()
    ...
    with st.container(border=True):
        ...
        if st.button("ارسال رابط التفعيل", type="primary", use_container_width=True):
            if user_email: token = secrets.token_urlsafe(32); expires = datetime.now() + timedelta(days=1); tokens_data["tokens"].append({"email": user_email, "token": token, "expires": expires.strftime("%Y-%m-%d %H:%M:%S"), "verified": False}); save_tokens(tokens_data)
    ...
    st.markdown("<div class='section-divider'></div>", unsafe_allow_html=True); st.markdown("### 📅 الجلسات خلال 7 ايام القادمة")
    today = datetime.now().date(); week_later = today + timedelta(days=7); df = pd.DataFrame(data["cases"])
    if not df.empty: df['تاريخ_جلسة'] = pd.to_datetime(df['تاريخ_جلسة'], errors='coerce').dt.date; upcoming = df[(df['تاريخ_جلسة'] >= today) & (df['تاريخ_جلسة'] <= week_later)]; verified_emails = [t['email'] for t in tokens_data['tokens'] if t['verified']]; st.info(f"عدد المشتركين المفعلين: {len(verified_emails)}")
    if not upcoming.empty: st.markdown("<div class='table-container'>", unsafe_allow_html=True); table_html = "<table class='case-table'><tr><th>م</th><th>الرقم والسنة</th><th>المحكمة</th><th>تاريخ الجلسة</th><th>السبب</th></tr>"
    for idx, row in enumerate(upcoming.iterrows(), 1): row = row[1]; رقم_كامل = f"{row['رقم']} لسنة {row['سنة']}"; محكمة = f"{row['محكمة_اسم']}"; table_html += f"<tr class='row1'><td>{idx}</td><td>{رقم_كامل}</td><td>{محكمة}</td><td>{row['تاريخ_جلسة']}</td><td>{row['سبب']}</td></tr>"
    table_html += "</table></div>"; st.markdown(table_html, unsafe_allow_html=True)
    else: st.info("مفيش جلسات خلال 7 ايام القادمة")
    else: st.warning("لا توجد قضايا مسجلة")

data = load_data(); today = datetime.now().strftime("%A, %d %B %Y")
st.markdown("""<style>@import url('https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;800&display=swap');html, body {font-family: 'Cairo', sans-serif; direction: rtl;}.stApp {background: linear-gradient(180deg, #0F1C2E 0%, #1A2F4F 100%);}.marquee-container {background: linear-gradient(90deg, #C9A961, #D4B96A); padding: 10px; border-radius: 8px; margin-bottom: 15px;}.marquee-text {color: #0F1C2E; font-weight: 800; font-size: 14px; white-space: nowrap; display: inline-block; animation: scroll-rtl 18s linear infinite;}@keyframes scroll-rtl {0% {transform: translateX(-100%);} 100% {transform: translateX(100%);}}.header-calm {background: linear-gradient(135deg, #1A2F4F, #2C4A73); padding: 18px; border-radius: 12px; text-align: center; border: 2px solid #C9A961; margin-bottom: 20px;}.header-calm h1 {color: #D4B96A; font-size: 30px; font-weight: 800; margin: 0;}.header-calm p {color: #E8E8E8; font-size: 13px; font-weight: 600; margin: 6px 0 0 0;}.section-divider {height: 4px; background: linear-gradient(90deg, transparent, #C9A961, transparent); margin: 15px 0;}label,.stTextInput label,.stSelectbox label,.stTextArea label,.stDateInput label {color: #D4B96A!important; font-weight: 700!important; font-size: 15px!important; text-align: right;}input, textarea, select {color: #0F1C2E!important; font-weight: 600; background-color: #FFFFFF!important;}div[data-testid="stButton"] > button {background: linear-gradient(135deg, #2C4A73, #3A5F8A)!important; color: #E8E8E8!important; width: 100%;border-radius: 12px; border: 2px solid #C9A961; font-weight: 700; font-size: 15px; height: 90px;}.btn-back button {height: 55px!important;}.btn-save button {background: linear-gradient(135deg, #C9A961, #D4B96A)!important; color: #0F1C2E!important; height: 50px!important; font-weight:800!important}.card {background: rgba(26,47,79,0.85); padding: 18px; border-radius: 12px; border: 2px solid #C9A961; margin-bottom: 18px;}.card-title {color: #D4B96A; font-weight: 800; font-size: 18px; margin-bottom: 15px; border-bottom: 2px solid #C9A961; padding-bottom: 10px; text-align: right;}.table-container {overflow-x: auto; border: 3px solid #C9A961; border-radius: 10px; background: white; margin-bottom:15px}.case-table {width: 100%; border-collapse: collapse; background: #FFFFFF; color: #0F1C2E; font-size: 14px;}.case-table th {background: linear-gradient(90deg, #D4B96A, #C9A961); color: #0F1C2E; padding: 12px 8px; text-align: center; font-weight: 800;}.case-table td {padding: 10px 8px; text-align: center; border: 1px solid #DDD; font-weight: 600;}.row1 {background:#F9F9F9}.row2 {background:#FFFFFF}.row-hey2a {background:#FFE5E5; font-weight:800}.row-judgment {background:#FFDCDC; font-weight:800}h2, h3, h4, p {color: #FFFFFF!important;}div[data-testid="stButton"] > button[kind="secondary"] {background: linear-gradient(135deg, #8B0000, #A52A2A)!important;color: #FFD700!important;border: 3px solid #FFD700!important;font-size: 18px!important;font-weight: 800!important;box-shadow: 0 0 15px #8B0000;}</style>""", unsafe_allow_html=True)
...
if st.session_state.page == "الرئيسية":
    ...
    st.markdown("<div class='section-divider'></div>", unsafe_allow_html=True); st.markdown("<h2 style='color:#C9A961; text-align:center'>تسجيل القضايا</h2>", unsafe_allow_html=True)
    ...
    with st.form("form_case"):
        st.markdown("<div class='card'><div class='card-title'>1- بيانات المحكمة</div>", unsafe_allow_html=True); محكمة_اسم = st.text_input("اسم المحكمة"); مأمورية = st.text_input("المأمورية") if نوع == "استئناف" else ""; st.markdown("</div>", unsafe_allow_html=True)
        st.markdown("<div class='card'><div class='card-title'>2- بيانات الدعوى</div>", unsafe_allow_html=True); col1, col2 = st.columns(2)
        with col1: رقم = st.text_input("رقم الدعوى / الاستئناف / الطعن")
        with col2: سنة = st.text_input("السنة القضائية"); دائرة = st.text_input("الدائرة"); st.markdown("</div>", unsafe_allow_html=True)
        st.markdown("<div class='card'><div class='card-title'>3- بيانات الخصوم</div>", unsafe_allow_html=True); col1, col2 = st.columns(2)
        with col1: مدعي = st.text_input("اسم المدعى / المستأنف / الطاعن")
        with col2: مدعي_عليه = st.text_input("اسم المدعى عليه / المستأنف ضده / المطعون ضده"); موضوع = st.text_area("موضوع الدعوى"); st.markdown("</div>", unsafe_allow_html=True)
        st.markdown("<div class='card'><div class='card-title'>4- بيانات الجلسة</div>", unsafe_allow_html=True); col1, col2 = st.columns(2)
        with col1: تاريخ_جلسة = st.date_input("تاريخ أول جلسة", value=datetime.now())
        with col2: الرول = st.text_input("الرول"); سبب = st.text_input("سبب الجلسة"); ملاحظات = st.text_area("ملاحظات"); st.markdown("</div>", unsafe_allow_html=True)
        st.markdown("<div class='card'><div class='card-title'>5- التنبيهات والمستندات</div>", unsafe_allow_html=True); تنبيه = st.checkbox("تفعيل التنبيهات عبر الواتس اب"); واتس = st.text_input("رقم هاتف واتس اب") if تنبيه else ""; col1, col2 = st.columns(2)
        with col1: مستند_نوع = st.selectbox("نوع المستند", ANWA3_MOSTANDAT)
        with col2: مستند_ملف = st.file_uploader("اختر الملف"); st.markdown("</div>", unsafe_allow_html=True); st.markdown("<div class='btn-save'>", unsafe_allow_html=True)
        if st.form_submit_button("حفظ القضية", use_container_width=True):
            ...
            else: new_case = {"id": len(data["cases"])+1, "نوع": نوع, "محكمة_اسم": محكمة_اسم, "مأمورية": مأمورية, "رقم": رقم, "سنة": سنة, "دائرة": دائرة, "مدعي": مدعي, "مدعي_عليه": مدعي_عليه, "موضوع": موضوع, "تاريخ_جلسة": str(تاريخ_جلسة), "الرول": الرول, "سبب": سبب, "ملاحظات": ملاحظات, "تنبيه": تنبيه, "واتس": واتس, "جلسات": [], "مستندات": [], "حالة": "متداولة"}
            ...
            data["cases"].append(new_case); save_data(data); st.success(f"✅ تم حفظ القضية رقم {رقم} لسنة {سنة}"); st.session_state.page = "حصر"; st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
elif st.session_state.page == "حصر":
    st.markdown("<div class='section-divider'></div>", unsafe_allow_html=True); st.markdown("<h2 style='color:#FFFFFF; text-align:center'>📊 الحصر العام الخارجي</h2>", unsafe_allow_html=True)
    if st.button("العودة للرئيسية", use_container_width=True): st.session_state.page = "الرئيسية"; st.rerun()
    if not data["cases"]: st.info("لا توجد قضايا مسجلة")
        ...
        total = len(sorted_cases); this_week = len([c for c in sorted_cases if c.get('تاريخ_جلسة') and datetime.strptime(c['تاريخ_جلسة'],'%Y-%m-%d') <= datetime.now() + timedelta(days=7)])
        st.info(f"📊 اجمالي القضايا: {total} | جلسات هذا الاسبوع: {this_week}")
        for idx, case in enumerate(sorted_cases, 1):
            رقم_كامل = f"{case.get('رقم','')} لسنة {case.get('سنة','')}"; محكمة_كاملة = f"{case.get('نوع','')} {case.get('محكمة_اسم','')}";
            ...
            st.markdown(table_html, unsafe_allow_html=True); c1, c2, c3 = st.columns([4,1,4])
            with c2:
...
# ============================================
# ============================================
# الجزء 3: صفحة التفاصيل وارسال الايميل
# ============================================
    ...
    if 'مستندات' not in case: case['مستندات'] = []
    st.markdown("<div class='section-divider'></div>", unsafe_allow_html=True); st.markdown(f"<h2 style='color:#FFFFFF; text-align:center'>📄 تفاصيل القضية رقم {case.get('رقم')} لسنة {case.get('سنة')}</h2>", unsafe_allow_html=True)
    ...
    st.markdown(table_html, unsafe_allow_html=True); st.markdown("<div class='section-divider'></div>", unsafe_allow_html=True); st.markdown("<h3 style='color:#C9A961'>📅 متابعة الجلسات</h3>", unsafe_allow_html=True)
    if len(case['جلسات']) > 0: جلسات_مرتبة = sorted(enumerate(case["جلسات"]), key=lambda x: x[1]['تاريخ']); html = "<table style='width:100%; border:3px solid #C9A961; background:#0A1428; border-radius:12px'><tr style='background:#C9A961; color:#000'><th>م</th><th>الرول</th><th>الجلسات</th><th>الإجراءات</th><th>ملاحظات</th><th>تحكم</th></tr>"
    for i, (idx, ج) in enumerate(جلسات_مرتبة, 1): لون = "#1E2A47" if i % 2 == 0 else "#142038"; html += f"<tr style='background:{لون}; color:#FFF'><td>{i}</td><td style='color:#FFD700'>{ج.get('الرول','-')}</td><td>{ج['تاريخ']}</td><td>{ج.get('سبب','-')}</td><td>{ج.get('ملاحظات','-')}</td><td><button onclick=\"window.location.href='?edit={idx}'\">✏️ تعديل</button></td></tr>"
    ...
    with st.expander("➕ اضافة جلسة" if edit_idx is None else f"✏️ تعديل جلسة سابقة رقم {int(edit_idx)+1}"):
        with st.form("session_form"):
            t_val = datetime.strptime(case['جلسات'][int(edit_idx)]['تاريخ'], '%Y-%m-%d').date() if edit_idx else datetime.now().date(); r_val = case['جلسات'][int(edit_idx)]['الرول'] if edit_idx else ""; s_val = case['جلسات'][int(edit_idx)]['سبب'] if edit_idx else ""; m_val = case['جلسات'][int(edit_idx)]['ملاحظات'] if edit_idx else ""; c1,c2 = st.columns(2)
            t = c1.date_input("تاريخ الجلسة", value=t_val); r = c2.text_input("الرول", value=r_val); s = st.text_input("سبب التأجيل", value=s_val); m = st.text_area("ملاحظات", value=m_val)
            if st.form_submit_button("حفظ"):
                ...
                else: case['جلسات'].append(session_data); send_session_alert_email(case, str(t), s) # سطر الارسال
                case['تاريخ_جلسة']=str(t); case['سبب']=s; save_data(data); st.success("✅ تم الحفظ وارسال التنبيهات"); st.rerun()
    st.markdown("<div class='section-divider'></div>", unsafe_allow_html=True); st.markdown("<h3 style='color:#C9A961'>📎 مستندات القضية</h3>", unsafe_allow_html=True)
# ============================================
# نهاية الجزء 3 ونهاية الكود كله
# ============================================
