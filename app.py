...
# دي القايمة اللي حضرتك كاتبها بالحرف
...
if st.session_state.page == "الرئيسية":
    ...
    with st.form("form_case"):
        ...
        if st.form_submit_button("حفظ القضية", use_container_width=True):
            if not رقم or not سنة: st.error("❌ من فضلك ادخل رقم الدعوى والسنة")
                ...
                st.session_state.page = "حصر"; st.rerun() # بيحول للحصر مباشرة
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if len(case['جلسات']) > 0:
        ...
        for i, (idx, ج) in enumerate(جلسات_مرتبة, 1):
            ...
            html += f"<tr style='background:{لون}; color:#FFF'><td>{i}</td><td style='color:#FFD700'>{ج.get('الرول','-')}</td><td>{ج['تاريخ']}</td><td>{ج.get('سبب','-')}</td><td>{ج.get('ملاحظات','-')}</td><td><button onclick=\"window.location.href='?edit={idx}'\">✏️</button></td></tr>"
        html += "</table>"
    ...
    if case['مستندات']:
        for i, مستند in enumerate(case['مستندات']):
            if 'مسار' in مستند and os.path.exists(مستند['مسار']):
                ...
                with c2: st.write(مستند['بيان'])
                with c3: st.write(مستند['اسم'])
                with c4:
    ...
    if 'حكم' not in case:
        with st.form("judgment_form"):
            ...
            if st.form_submit_button("حفظ الحكم واغلاق القضية"):
                ...
                case['حالة'] = 'منتهية'
                case['جلسات'].append({'تاريخ':str(تاريخ_حكم),'الرول':'-','سبب':f'الحكم - مسندة لـ {مسندة_ل}','ملاحظات':منطوق_الحكم}) # بتتسجل في الجدول
