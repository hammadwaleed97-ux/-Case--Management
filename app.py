...
# ==================================================================
# ==================================================================
# ================== بداية الجزء 2: الحصر والعرض ==================
# ==================================================================

elif st.session_state.page == "حصر":
    st.markdown("<div class='section-divider'></div>", unsafe_allow_html=True)
    st.markdown("<h2 style='color:#C9A961; text-align:center'>الحصر العام للقضايا</h2>", unsafe_allow_html=True)
    if st.button("العودة للرئيسية", use_container_width=True): st.session_state.page = "الرئيسية"; st.rerun()

    if not data["cases"]:
        st.warning("لا توجد قضايا مسجلة بعد")
    else:
        df = pd.DataFrame(data["cases"])

        # بنصلح اي قضية قديمة ناقصها "حالة"
        if 'حالة' not in df.columns:
            df['حالة'] = 'متداولة'
        df['حالة'] = df['حالة'].fillna('متداولة')
    ...
    case_id = int(query_params.get("id", [st.session_state.selected_case_id])[0])
    case = next((c for c in data["cases"] if c["id"] == case_id), None)

    if case:
        st.markdown("<div class='section-divider'></div>", unsafe_allow_html=True)
        st.markdown(f"<h2 style='color:#C9A961; text-align:center'>عرض القضية رقم {case['رقم']}</h2>", unsafe_allow_html=True)
        col1, col2 = st.columns(2)
        with col1:
            if st.button("العودة للحصر", use_container_width=True): st.session_state.page = "حصر"; st.rerun()
        with col2:
            if st.button("تعديل القضية", use_container_width=True): st.session_state.page = "تعديل"; st.session_state.selected_case_id = case_id; st.rerun()

        st.markdown(f"<div class='card'><div class='card-title'>بيانات القضية</div><p><b>نوع:</b> {case['نوع']}</p><p><b>المحكمة:</b> {case['محكمة_اسم']}</p><p><b>المدعي:</b> {case['مدعي']}</p><p><b>المدعي عليه:</b> {case['مدعي_عليه']}</p><p><b>الموضوع:</b> {case['موضوع']}</p><p><b>الحالة:</b> {case.get('حالة', 'متداولة')}</p></div>", unsafe_allow_html=True)

        st.markdown("<div class='card'><div class='card-title'>الجلسات</div>", unsafe_allow_html=True)
        for j in case.get('جلسات', []):
            st.markdown(f"<p>📅 {j['تاريخ']} - الرول: {j['الرول']} - {j['سبب']}</p>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if case:
        st.markdown("<div class='section-divider'></div>", unsafe_allow_html=True)
        st.markdown(f"<h2 style='color:#C9A961; text-align:center'>تعديل القضية رقم {case['رقم']}</h2>", unsafe_allow_html=True)
        if st.button("العودة للعرض", use_container_width=True): st.session_state.page = "عرض"; st.rerun()

        with st.form("form_edit"):
            case['محكمة_اسم'] = st.text_input("اسم المحكمة", case['محكمة_اسم'])
            case['دائرة'] = st.text_input("الدائرة", case['دائرة'])
            case['موضوع'] = st.text_area("موضوع الدعوى", case['موضوع'])
            حالة_حالية = case.get('حالة', 'متداولة')
            case['حالة'] = st.selectbox("حالة القضية", ["متداولة", "هيئة", "حكم"], index=["متداولة", "هيئة", "حكم"].index(حالة_حالية))
            st.markdown("<div class='btn-save'>", unsafe_allow_html=True)
            if st.form_submit_button("حفظ التعديلات", use_container_width=True):
                save_data(data)
                st.success("✅ تم حفظ التعديلات")
                st.session_state.page = "عرض"; st.rerun()
            st.markdown("</div>", unsafe_allow_html=True)

# ==================================================================
# ================== نهاية الجزء 2: الحصر والعرض ==================
# ==================================================================
