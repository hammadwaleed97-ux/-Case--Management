        ...
        if not search_value.strip(): st.error("اكتب حاجة تبحث بيها")
            ...
            if not results: st.warning("لم يتم العثور على نتائج")
                ...
                for idx, case in enumerate(results, 1):
                    ...
                    if case.get('حالة') == 'منتهية':
                        row_class = "row-judgment"; حالة_لون = "#FF5252"; مكان = "📁 الأرشيف"; الصفحة_المطلوبة = "ارشيف" # <-- اتعدل
                    else:
                        row_class = "row1" if idx % 2 == 1 else "row2"; حالة_لون = "#4CAF50"; مكان = "📋 الحصر العام"; الصفحة_المطلوبة = "الحصر"
