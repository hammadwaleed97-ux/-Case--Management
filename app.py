...
# ================================================
    ...
    def create_pdf(cases_list, filename, report_type, region, title):
        ...
        p.drawCentredString(width/2, y, "الهيئة القومية للتأمين الاجتماعى"); y -= 20
        p.drawCentredString(width/2, y, "الإدارة المركزية للإدارات القانونية"); y -= 20
        p.drawCentredString(width/2, y, "الإدارة العامة للقضايا"); y -= 20
        p.drawCentredString(width/2, y, f"ديوان عام {region}"); y -= 30
        p.drawCentredString(width/2, y, title); y -= 30

        if cases_list:
            data_table = [['م']]
            if report_type == "متداولة":
                data_table[0].extend(['رقم القضية', 'السنة', 'الدائرة والنوع', 'المحكمة', 'الخصوم', 'الموضوع', 'اخر جلسة', 'ملاحظات'])
                for i, c in enumerate(cases_list, 1):
                    data_table.append([i, c.get('رقم',''), c.get('سنة',''), f"{c.get('دائرة','')} {c.get('نوع','')}", c.get('محكمة_اسم',''), f"{c.get('مدعي','')} vs {c.get('مدعي_عليه','')}", c.get('موضوع',''), c.get('تاريخ_جلسة',''), c.get('ملاحظات','')])
            else:
                data_table[0].extend(['رقم القضية', 'السنة', 'الدائرة والنوع', 'المحكمة', 'الخصوم', 'الموضوع', 'تاريخ الحكم', 'المنطوق', 'النتيجة', 'ملاحظات'])
                for i, c in enumerate(cases_list, 1):
                    data_table.append([i, c.get('رقم',''), c.get('سنة',''), f"{c.get('دائرة','')} {c.get('نوع','')}", c.get('محكمة_اسم',''), f"{c.get('مدعي','')} vs {c.get('مدعي_عليه','')}", c.get('موضوع',''), c.get('تاريخ_الحكم',''), c.get('منطوق_الحكم',''), c.get('مسندة_ل_الحكم',''), c.get('ملاحظات','')])

            t = Table(data_table)
            t.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.gold),('GRID',(0,0),(-1,-1),1,colors.black)]))
    ...
    def create_word(cases_list, filename, report_type, region, title):
        ...
        if cases_list:
            if report_type == "متداولة":
                ...
                hdr[0].text, hdr[1].text, hdr[2].text, hdr[3].text, hdr[4].text, hdr[5].text, hdr[6].text, hdr[7].text, hdr[8].text = 'م','رقم القضية','السنة','الدائرة والنوع','المحكمة','الخصوم','الموضوع','اخر جلسة','ملاحظات'
                for i, c in enumerate(cases_list, 1):
                    row = table.add_row().cells
                    row[0].text, row[1].text, row[2].text, row[3].text, row[4].text, row[5].text, row[6].text, row[7].text, row[8].text = str(i), c.get('رقم',''), c.get('سنة',''), f"{c.get('دائرة','')} {c.get('نوع','')}", c.get('محكمة_اسم',''), f"{c.get('مدعي','')} vs {c.get('مدعي_عليه','')}", c.get('موضوع',''), c.get('تاريخ_جلسة',''), c.get('ملاحظات','')
            else:
                table = doc.add_table(rows=1, cols=11)
                hdr = table.rows[0].cells
                hdr[0].text, hdr[1].text, hdr[2].text, hdr[3].text, hdr[4].text, hdr[5].text, hdr[6].text, hdr[7].text, hdr[8].text, hdr[9].text, hdr[10].text = 'م','رقم القضية','السنة','الدائرة والنوع','المحكمة','الخصوم','الموضوع','تاريخ الحكم','المنطوق','النتيجة','ملاحظات'
                for i, c in enumerate(cases_list, 1):
                    row = table.add_row().cells
                    row[0].text, row[1].text, row[2].text, row[3].text, row[4].text, row[5].text, row[6].text, row[7].text, row[8].text, row[9].text, row[10].text = str(i), c.get('رقم',''), c.get('سنة',''), f"{c.get('دائرة','')} {c.get('نوع','')}", c.get('محكمة_اسم',''), f"{c.get('مدعي','')} vs {c.get('مدعي_عليه','')}", c.get('موضوع',''), c.get('تاريخ_الحكم',''), c.get('منطوق_الحكم',''), c.get('مسندة_ل_الحكم',''), c.get('ملاحظات','')
    ...
    def export_buttons(cases_list, filename, report_type, region, title):
        ...
        excel_data = df_export.to_csv(index=False, encoding='utf-8-sig') if not df_export.empty else "لا توجد بيانات"
        ...
        with c5:
            st.download_button("📊 تحميل Excel", data=excel_data, file_name=f"{filename}.xlsx", use_container_width=True, key=f"dl_excel_{filename}", mime="text/csv")
