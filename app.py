...
# ================================================
    ...
    if alert_appeals:
        ...
        for i, case in enumerate(alert_appeals):
            if case['days_left_appeal'] > 0: status = f"متبقي {case['days_left_appeal']} يوم"
            elif case['days_left_appeال'] == 0: status = "اليوم اخر فرصة للتنبيه"
            else: status = f"انتهى من {abs(case['days_left_appeal'])} يوم"
    ...
    if st.button("📧 ارسال التنبيهات الان للايميل", use_container_width=True, type="primary"):
        ...
        if not receiver: st.error("لازم تكتب الايميل المستقبل")
            ...
            ok, msg = send_email(receiver, f"تنبيهات القضايا - {today}", body)
