...
# ================================================
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📊 بيان الدعاوى المتداولة", "⚖️ بيان الاحكام", "👤 بيان العضو", "📈 الإحصائيات"])
    ...
    def generate_pdf_report(cases, report_type, region, from_date, to_date, lawyer):
        ...
        style_ar = ParagraphStyle('Arabic', parent=styles['Normal'], fontName='Cairo', fontSize=10, alignment=2, leading=16)
        style_header = ParagraphStyle('Header', parent=styles['Heading2'], fontName='Cairo', fontSize=14, alignment=1, textColor=colors.HexColor('#D4AF37'))

        title = arabic(f"الهيئة القومية للتأمين الاجتماعي - {report_type}")
        elements.append(Paragraph(title, style_header))
        elements.append(Spacer(1, 12))

        table_data = []
        if report_type == "الدعاوى المتداولة" or report_type == "بيان العضو":
            headers = [arabic(h) for h in ['م', 'رقم القضية', 'السنة', 'الدائرة', 'المحكمة', 'الخصوم', 'الموضوع', 'اخر جلسة']]
            table_data.append(headers)
            for i, c in enumerate(cases, 1):
                محكمة = c.get('محكمة_اسم','')
                if c.get('مأمورية'): محكمة += f" مأمورية {c.get('مأمورية')}"
                row = [arabic(str(i)), arabic(c.get('رقم','')), arabic(c.get('سنة','')), arabic(c.get('دائرة','')), arabic(محكمة), arabic(c.get('مدعي','') + " ضد " + c.get('مدعي_عليه','')), arabic(c.get('موضوع','')), arabic(c.get('تاريخ_جلسة',''))]
                table_data.append(row)
        else: # الاحكام
            headers = [arabic(h) for h in ['م', 'رقم القضية', 'السنة', 'تاريخ الحكم', 'المنطوق', 'النتيجة']]
            table_data.append(headers)
            for i, c in enumerate(cases, 1):
        ...
        t.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), colors.HexColor('#D4AF37')), ('TEXTCOLOR', (0,0), (-1,0), colors.whitesmoke), ('ALIGN', (0,0), (-1,-1), 'CENTER'), ('FONTNAME', (0,0), (-1,-1), 'Cairo'), ('FONTSIZE', (0,0), (-1,-1), 8), ('GRID', (0,0), (-1,-1), 1, colors.black)]))
    ...
    with tab1:
        ...
        if st.button("🔍 عرض بيان الدعاوى المتداولة", use_container_width=True, type="primary", key="show1"):
            ...
            # زر التصدير تحت التقرير
            pdf_buffer = generate_pdf_report(cases, "الدعاوى المتداولة", region, from_date, to_date, lawyer)
            ...
            st.markdown(f"<p style='text-align:right; color:#D4AF37; margin-top:30px; font-size:16px; font-family:Cairo'>تفضلوا بقبول وافر الاحترام<br><br>عضو الادارة.................. مدير الإدارة..................<br>تحر في {datetime.now().strftime('%Y-%m-%d')}</p>", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if st.button("🔍 عرض بيان الاحكام", use_container_width=True, type="primary", key="show2"):
            ...
            # زر التصدير تحت التقرير
            pdf_buffer = generate_pdf_report(cases, "الاحكام", region2, from_date2, to_date2, lawyer2)
            ...
            st.markdown(f"<p style='text-align:right; color:#FF5252; margin-top:30px; font-size:16px; font-family:Cairo'>تفضلوا بقبول وافر الاحترام<br><br>عضو الادارة.................. مدير الإدارة..................<br>تحرر في {datetime.now().strftime('%Y-%m-%d')}</p>", unsafe_allow_html=True)
    ...
    with tab3:
        ...
        region3 = st.text_input("ديوان عام منطقة", key="region3")
        col1, col2 = st.columns(2)
        with col1: from_date3 = st.date_input("من الفترة", key="from3")
        with col2: to_date3 = st.date_input("حتى الفترة", key="to3")
        عضو_الاسم = st.text_input("اسم العضو / المحامي", key="lawyer3")
        st.markdown("</div>", unsafe_allow_html=True)

        if st.button("🔍 عرض بيان العضو", use_container_width=True, type="primary", key="show3"):
            # نجيب كل قضايا العضو متداولة + منتهية
            cases = [c for c in data["cases"] if عضو_الاسم in str(c.get('المحامي_المسند','')) or عضو_الاسم in str(c.get('lawyer',''))]
            if cases: cases = [c for c in cases if c.get('تاريخ_جلسة') and from_date3 <= datetime.strptime(c['تاريخ_جلسة'], '%Y-%m-%d').date() <= to_date3]
            cases = sorted(cases, key=lambda x: x.get("تاريخ_جلسة","9999-12-31"), reverse=True)

            report_header(region3, f"بيان بقضايا الاستاذ/ {عضو_الاسم} خلال الفترة من {from_date3} حتى {to_date3}")

            if not cases: st.warning("لا توجد قضايا مسندة للعضو في الفترة المحددة")
            else:
                html = "<table class='case-table'><tr><th>م</th><th>رقم القضية</th><th>السنة</th><th>الحالة</th><th>الدائرة</th><th>المحكمة</th><th>الموضوع</th><th>اخر اجراء</th></tr>"
                for i, c in enumerate(cases, 1):
                    محكمة = f"{c.get('محكمة_اسم','')}"
                    if c.get('مأمورية'): محكمة += f"<br>مأمورية {c.get('مأمورية')}"
            ...
            # زر التصدير تحت التقرير
            pdf_buffer = generate_pdf_report(cases, "بيان العضو", region3, from_date3, to_date3, عضو_الاسم)
            st.download_button("⬇️ تحميل التقرير PDF", data=pdf_buffer, file_name=f"report_member_{datetime.now().strftime('%Y%m%d')}.pdf", mime="application/pdf", use_container_width=True)
            st.markdown(f"<p style='text-align:right; color:#4CAF50; margin-top:30px; font-size:16px; font-family:Cairo'>تفضلوا بقبول وافر الاحترام<br><br>عضو الادارة.................. مدير الإدارة..................<br>تحرر في {datetime.now().strftime('%Y-%m-%d')}</p>", unsafe_allow_html=True)
