    ...
    def generate_pdf_report(cases, report_type, region, from_date, to_date, lawyer):
        ...
        if report_type == "الدعاوى المتداولة":
            ...
            for i, c in enumerate(cases, 1):
                row = [
                    arabic(str(i)), arabic(c.get('رقم','')), arabic(c.get('سنة','')), 
                    arabic(c.get('دائرة','')), arabic(c.get('محكمة_اسم','')), 
                    arabic(c.get('مدعي','') + " ضد " + c.get('مدعي_عليه','')),
                    arabic(c.get('موضوع','')), arabic(c.get('تاريخ_جلسة',''))
                ]
            ...
            for i, c in enumerate(cases, 1):
                row = [
                    arabic(str(i)), arabic(c.get('رقم','')), arabic(c.get('سنة','')),
                    arabic(c.get('تاريخ_الحكم','')), arabic(c.get('منطوق_الحكم','')), arabic(c.get('مسندة_ل_الحكم',''))
                ]
                table_data.append(row)
    ...
    with tab1:
        ...
        if st.button("🔍 عرض بيان الدعاوى المتداولة", use_container_width=True, type="primary", key="show1"):
            ...
            if not cases: st.warning("لا توجد دعاوى متداولة في الفترة المحددة")
                ...
                st.download_button("⬇️ تحميل PDF", data=pdf_buffer, file_name=f"report_motadawla_{datetime.now().strftime('%Y%m%d')}.pdf", mime="application/pdf")
