...
import json, os, bcrypt, smtplib, random, io, requests, base64
...
def show_banners():
    ...
    for banner in banners:
        try:
            expire_at = datetime.fromisoformat(str(banner.get("expire_at", "9999-12-31T23:59:59")))
        except Exception:
...
if st.session_state.page == "الرئيسية":
    ...
    if st.session_state.user.get("role") == "admin":
        ...
        with c_admin1:
            if st.button("👥 إدارة الأعضاء", use_container_width=True):
                st.session_state.page = "ادارة_الاعضاء"
                st.rerun()
        with c_admin2:
            if st.button("🪪 استخراج عضوية جديدة", use_container_width=True):
                st.session_state.page = "extract_member"
                st.rerun()

    c_user1, c_user2, c_user3 = st.columns(3)
    with c_user1:
        if st.button("🔑 تغيير كلمة السر", use_container_width=True):
            st.session_state.page = "change_password"
            st.rerun()
    with c_user2:
        if st.button("📧 تأكيد البريد", use_container_width=True):
