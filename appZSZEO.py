...
button [data-testid="stIconMaterial"], button span, button p { opacity:1 !important; visibility:visible !important; color:#fff !important; fill:#fff !important; }
...
# ===== التخزين الدائم للملفات: Supabase Storage =====
SUPABASE_STORAGE_BUCKET = _secret("SUPABASE_STORAGE_BUCKET", "legal-files")
_STORAGE_READY = False

def _storage_headers(content_type=None, upsert=True):
...
def _ensure_storage_bucket():
    ...
    if not CLOUD_ENABLED or _STORAGE_READY:
...
def _storage_upload_bytes(path, data_bytes, content_type="application/octet-stream"):
    if not data_bytes or not _ensure_storage_bucket():
        return None
    path = str(path).lstrip("/")
    try:
        r = requests.post(
            f"{SUPABASE_URL}/storage/v1/object/{SUPABASE_STORAGE_BUCKET}/{path}",
            headers=_storage_headers(content_type, True),
            data=data_bytes,
            timeout=45,
        )
        return path if r.status_code in (200, 201) else None
    except Exception:
        return None

def _storage_download_bytes(path):
    if not CLOUD_ENABLED or not path:
        return None
    try:
        r = requests.get(
            f"{SUPABASE_URL}/storage/v1/object/{SUPABASE_STORAGE_BUCKET}/{str(path).lstrip('/')}",
...
def _safe_storage_name(name):
    name = os.path.basename(str(name or "مرفق"))
    name = re.sub(r"[^\w\-.\u0600-\u06FF ]+", "_", name, flags=re.UNICODE).strip()
    return name or "مرفق"

def _sync_storage_payload(payload, namespace="data"):
    """يرفع المرفقات والصور الموجودة في البيانات إلى Supabase Storage ويضيف storage_path."""
    if not CLOUD_ENABLED or payload is None:
        return False
    changed = False
    ...
    def walk(obj):
        ...
        # مرفقات الرسائل واليافطات: {data, name, type}
        if obj.get("data") and obj.get("name") and not obj.get("storage_path"):
            try:
                raw = base64.b64decode(obj.get("data") or "", validate=False)
                if raw:
                    ident = re.sub(r"[^A-Za-z0-9_-]", "", str(obj.get("id") or secrets.token_hex(5)))
                    path = f"{namespace}/attachments/{ident}_{_safe_storage_name(obj.get('name'))}"
                    saved = _storage_upload_bytes(path, raw, obj.get("type") or "application/octet-stream")
                    if saved:
                        obj["storage_path"] = saved
                        changed = True
            except Exception:
                pass

        # مستندات القضايا: {محتوى, نوع}
        if obj.get("محتوى") and obj.get("نوع") and not obj.get("storage_path"):
            try:
                raw = base64.b64decode(obj.get("محتوى") or "", validate=False)
                if raw:
                    path = f"{namespace}/documents/{secrets.token_hex(5)}_{_safe_storage_name(obj.get('نوع'))}"
                    saved = _storage_upload_bytes(path, raw, "application/octet-stream")
                    if saved:
                        obj["storage_path"] = saved
                        changed = True
            except Exception:
        ...
        # الصور داخل الرسائل
        if obj.get("image_data") and not obj.get("image_storage_path"):
            try:
                raw = base64.b64decode(obj.get("image_data") or "", validate=False)
                if raw:
                    typ = obj.get("image_type") or "image/png"
                    ext = typ.split("/",1)[1].split(";",1)[0] if "/" in typ else "png"
                    path = f"{namespace}/images/{secrets.token_hex(5)}.{_safe_storage_name(ext)}"
                    saved = _storage_upload_bytes(path, raw, typ)
                    if saved:
                        obj["image_storage_path"] = saved
                        changed = True
            except Exception:
                pass

        # صور العضويات والمرسلين
        for data_key, type_key, storage_key, folder in (
            ("profile_photo", "profile_photo_type", "profile_photo_storage_path", "members"),
            ("sender_photo", "sender_photo_type", "sender_photo_storage_path", "messages"),
        ):
            if obj.get(data_key) and not obj.get(storage_key):
                try:
                    raw = base64.b64decode(obj.get(data_key) or "", validate=False)
                    if raw:
                        typ = obj.get(type_key) or "image/png"
                        ext = typ.split("/",1)[1].split(";",1)[0] if "/" in typ else "png"
                        ident = re.sub(r"[^A-Za-z0-9_-]", "", str(obj.get("id") or secrets.token_hex(5)))
                        path = f"{namespace}/{folder}/{ident}.{_safe_storage_name(ext)}"
                        saved = _storage_upload_bytes(path, raw, typ)
                        if saved:
                            obj[storage_key] = saved
                            changed = True
                except Exception:
                    pass
    ...
    if cloud and cloud.get("payload") is not None:
        try:
            ...
            if local_obj is not None and os.path.exists(local_path):
                ...
                local_obj=cloud_obj
            if _sync_storage_payload(local_obj, key):
...
def save_json_persistent(key, local_path, payload):
    _sync_storage_payload(payload, key)
    os.makedirs(os.path.dirname(local_path) or ".", exist_ok=True)
    tmp=local_path+".tmp"
    with open(tmp,"w",encoding="utf-8") as f: json.dump(payload,f,ensure_ascii=False,indent=4)
    os.replace(tmp,local_path)
    return _cloud_put(key,payload) if CLOUD_ENABLED else True
...
def _cached_users_snapshot():
    return load_users()

@st.cache_data(ttl=20, show_spinner=False)
def _cached_banners_snapshot():
    return load_banners()


def check_login(username, password):
    users = _cached_users_snapshot()
    changed = False
    for u in users:
...
def membership_job_title(user):
    job = str(user.get("job") or user.get("وظيفة") or user.get("role_name") or "")
    return "مدير عام (قيادى مناطق)" if job.strip() == "مدير عام" else job
...
def manage_users_page():
    ...
    if st.button("✏️ تعديل بيانات العضويات", key="open_membership_editor", use_container_width=True):
        st.session_state.page = "تعديل_بيانات_العضويات"
        st.rerun()
    ...
    for area in sorted(set((u.get("insurance_area") or "غير محددة").strip() or "غير محددة" for u in users), key=lambda x:x.casefold()):
        ...
        for i,u in enumerate(area_users):
            with cols[i % 6]:
                ...
                job_value = membership_job_title(u) or role_label(u.get("role"))
...
def admin_control_page():
    ...
    with tab_messages:
        ...
        users = _cached_users_snapshot()
        banners = _cached_banners_snapshot()
        now = datetime.now()
        ...
        with st.form("admin_banner_add_v2"):
            ...
            banner_files = st.file_uploader("مرفقات البيان", type=None, accept_multiple_files=True, key="banner_files_add_v4")
            ...
            if st.form_submit_button("🚀 نشر الرسالة / البيان", use_container_width=True, type="primary"):
                if not text.strip():
                    ...
                    banner = {
                        ...
                        "image_data": "",
                        "image_type": "",
                        "attachments": [{"data": base64.b64encode(f.getvalue()).decode("utf-8"), "name": f.name, "type": f.type or "application/octet-stream"} for f in (banner_files or [])]
                    }
        ...
        for i,b in enumerate(banners):
            ...
            with st.container(border=True):
                st.markdown(
                    ...
                    f"<div style='color:#ddd;'>تنتهي: {expire_raw or 'غير محدد'} | الحالة: {'منتهية' if expired else 'فعالة'}</div>"
                    + "".join(f"<div style='color:#fff;margin-top:5px;'>📎 {html.escape(str(a.get('name') or 'مرفق'))}</div>" for a in (b.get('attachments') or []))
                    + "</div>",
                    unsafe_allow_html=True
                )
                e1,e2 = st.columns(2)
                ...
                if st.session_state.get("edit_banner_id") == b.get("id"):
                    with st.form(f"edit_banner_form_{b.get('id',i)}"):
                        ...
                        nfiles = st.file_uploader("مرفقات البيان", type=None, accept_multiple_files=True, key=f"eimg_{b.get('id',i)}")
                        ...
                        with ec1:
                            if st.form_submit_button("💾 حفظ التعديل", use_container_width=True):
                                ...
                                b["visible_to"] = nvis
                                if nfiles:
                                    b["attachments"] = b.get("attachments") or []
                                    b["attachments"].extend([{"data": base64.b64encode(f.getvalue()).decode("utf-8"), "name": f.name, "type": f.type or "application/octet-stream"} for f in nfiles])
    ...
    with tab_search:
        st.markdown("<h3 style='color:#D4AF37;'>🔎 البحث عن عضو أو مدير إدارة أو مدير عامأو قيادي</h3>", unsafe_allow_html=True)
...
def report_download_name(prefix, member_name="", specialization="", region="", ext=""):
    parts = [str(prefix or "").strip(), str(member_name or "").strip(), str(specialization or "").strip(), str(region or "").strip()]
    clean = []
    for p in parts:
        p = re.sub(r'[\\/:*?"<>|]+', "_", p)
        if p:
            clean.append(p)
    name = " - ".join(clean) or "تقرير"
    return name + (f".{ext}" if ext else "")

def to_excel(df, title="التقرير"):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        for col in ws.columns:
            ...
            ws.column_dimensions[letter].width = min(max(length + 3, 14), 45)
    return output.getvalue()

def to_word(df, title, region, member_name="", specialization="", manager_name="", general_manager=""):
    doc = Document()
    def rtl(text, size=11, bold=False, align=WD_ALIGN_PARAGRAPH.RIGHT):
        p=doc.add_paragraph(); p.alignment=align
        p._p.get_or_add_pPr().append(OxmlElement("w:bidi"))
        r=p.add_run(str(text)); r.font.name="Arial"; r.font.size=Pt(size); r.bold=bold
        return p
    rtl("الهيئة القومية للتأمين الاجتماعي",16,True,WD_ALIGN_PARAGRAPH.CENTER)
    rtl("الإدارة المركزية للإدارات القانونية",13,True,WD_ALIGN_PARAGRAPH.CENTER)
    rtl("الإدارة العامة للقضايا",13,True,WD_ALIGN_PARAGRAPH.CENTER)
    if region: rtl(f"ديوان عام منطقة {region}",12,True,WD_ALIGN_PARAGRAPH.CENTER)
    rtl(title,13,True,WD_ALIGN_PARAGRAPH.CENTER)
    table=doc.add_table(rows=1, cols=max(1,len(df.columns))); table.style="Table Grid"
    for i,col in enumerate(df.columns):
        cell=table.rows[0].cells[i]; cell.text=str(col); cell.paragraphs[0].alignment=WD_ALIGN_PARAGRAPH.RIGHT
        cell.paragraphs[0]._p.get_or_add_pPr().append(OxmlElement("w:bidi"))
    for _,row in df.iterrows():
        cells=table.add_row().cells
        for i,val in enumerate(row):
            cells[i].text=str(val); cells[i].paragraphs[0].alignment=WD_ALIGN_PARAGRAPH.RIGHT
            cells[i].paragraphs[0]._p.get_or_add_pPr().append(OxmlElement("w:bidi"))
    rtl(f"تحريرًا في: {datetime.now().strftime('%d-%m-%Y')}",10)
    rtl("وتفضلوا بقبول وافر الاحترام والتقدير،",11,True)
    rtl("عضو الإدارة القانونية",10,True); rtl(member_name,10); rtl("....................",10)
    rtl("مدير إدارة القضايا",10,True); rtl(manager_name,10); rtl("....................",10)
    rtl("مدير عام الإدارات القانونية",10,True); rtl(general_manager,10); rtl("....................",10)
    out=io.BytesIO(); doc.save(out); return out.getvalue()


def to_pdf(df, title, region="", member_name="", specialization="", manager_name="", general_manager_name="", *args, **kwargs):
    """PDF عربي: ترتيب الأعمدة من اليمين إلى اليسار بدون عكس بصري للجدول."""
    pdf = FPDF(orientation="L", unit="mm", format="A4")
    pdf.add_page()
    pdf.set_auto_page_break(auto=True, margin=15)

    font_path = _report_font_path() if "_report_font_path" in globals() else None
    ...
    def ar(v):
        try:
            return fix_arabic("" if v is None else str(v))
        except Exception:
    ...
    for text in (
        ...
        pdf.cell(0, 7, ar(text), 0, 1, "C")
    if s(region):
        pdf.cell(0, 7, ar(f"ديوان عام منطقة {s(region)}"), 0, 1, "C")

    pdf.ln(2)
    pdf.set_font(font_name, "", 9.5)
    if s(title):
        pdf.multi_cell(pdf.w - 24, 6, ar(s(title)), 0, "C")
    pdf.ln(2)

    # ترتيب DataFrame هو ترتيب القراءة العربي:
    # الموضوع ← الخصوم ← المحكمة ← السنة ← الرقم ← م
    # أول عمود يتم رسمه يدويًا من أقصى اليمين.
    if df is not None and len(getattr(df, "columns", [])):
        ...
        for col in columns:
            ...
            if "الموضوع" in name:
                ...
                widths.append(42)
            elif "المحكمة" in name or "الدائرة" in name:
                widths.append(62)
            elif "السنة" in name:
                widths.append(28)
            elif "رقم" in name:
                widths.append(28)
            elif name == "م" or name.startswith("م "):
                widths.append(18)
            else:
                widths.append(35)
        ...
        def wrap(v, w):
            ...
            for word in text.split():
                if pdf.get_string_width(ar(word)) > w - 3:
                    if cur:
                        out.append(cur)
                        cur = ""
                    piece = ""
                    for ch in word:
                        if pdf.get_string_width(ar(piece + ch)) <= w - 3:
                            piece += ch
                        else:
                            if piece:
                                out.append(piece)
                    ...
                    if pdf.get_string_width(ar(test)) <= w - 3:
                        cur = test
                    else:
                        if cur:
                            out.append(cur)
        ...
        def rtl_row(values, header=False):
            y = pdf.get_y()
            x_right = page_right
            line_h = 5.2 if header else 5.0
            wrapped = [wrap(v, w) for v, w in zip(values, widths)]
            row_h = max(line_h, min(35, max(len(x) for x in wrapped) * line_h))

            # لا يوجد reversed هنا.
            # أول عمود في DataFrame = أقصى اليمين فعليًا.
            for lines, w in zip(wrapped, widths):
                x_right -= w
                pdf.rect(x_right, y, w, row_h)
                pdf.set_xy(x_right + 1, y + 1)
                pdf.multi_cell(w - 2, line_h, ar("\n".join(lines)), 0, "C")

            pdf.set_y(y + row_h)

        pdf.set_font(font_name, "", 6.5)
        rtl_row(columns, True)
        for row in rows:
            rtl_row(row, False)

    pdf.ln(7)
    pdf.set_font(font_name, "", 10)
    pdf.cell(0, 8, ar("وتفضلوا بقبول وافر الاحترام والتقدير"), 0, 1, "C")
    pdf.ln(7)
    ...
    pdf.set_xy(pdf.w - margin - box_w, y)
    member = ["عضو الإدارة القانونية"]
    if s(member_name):
        member.append(s(member_name))
    member.append(".................................")
    member.append(f"تحرر في: {datetime.now().strftime('%d/%m/%Y')}")
    pdf.multi_cell(box_w, 7, ar("\n".join(member)), 0, "C")

    pdf.set_xy(margin, y)
    manager = ["مدير إدارة القضايا"]
    if s(manager_name):
        manager.append(s(manager_name))
    manager.append(".................................")
    pdf.multi_cell(box_w, 7, ar("\n".join(manager)), 0, "C")

    pdf.set_y(y + 40)
    pdf.cell(0, 7, ar("مدير عام الإدارات القانونية"), 0, 1, "C")
    if s(general_manager_name):
        pdf.cell(0, 7, ar(general_manager_name), 0, 1, "C")
    pdf.cell(0, 7, ar("................................................"), 0, 1, "C")
...
.stButton>button{width:100%!important;max-width:400px!important;min-height:76px!important;border-radius:15px!important;font-size:18px!important;font-weight:900!important;padding:16px!important;color:#fff!important;background:linear-gradient(135deg,#1a2b4b,#263f69)!important;border:1px solid #D4AF37!important;box-shadow:0 8px 18px rgba(0,0,0,.22)!important;}
.stButton>button:hover{border-color:#FFD700!important;transform:translateY(-1px);}
...
def edit_membership_data_page():
    if st.button("⬅️ العودة للصفحة الرئيسية", key="back_from_membership_edit", use_container_width=True):
        st.session_state.page = "الرئيسية"
        st.rerun()
    ...
    if not st.session_state.get("user") or st.session_state.user.get("role") != "admin":
        st.error("هذه الصفحة متاحة للادمن فقط")
        return
    st.markdown("## تعديل بيانات العضويات")
    users = load_users()
    active_users = [u for u in users if u.get("status") != "deleted"]
    if not active_users:
        st.info("لا توجد عضويات.")
        return

    labels = []
    mapping = {}
    for u in active_users:
        label = f"{u.get('display_name') or u.get('username') or ''} — {u.get('job_title') or role_label(u.get('role'))}"
        labels.append(label)
        mapping[label] = u

    selected = st.selectbox("اختر العضوية", labels, key="membership_edit_select")
    target = mapping[selected]

    jobs = ["عضو قانونى", "مدير إدارة", "مدير عام", "قيادي مركز رئيسى 1", "قيادي مركز رئيسى 2", "قيادي مركز رئيسى 3"]
    current_job = str(target.get("job_title") or "")
    with st.form("membership_edit_form"):
        name = st.text_input("الاسم", value=str(target.get("display_name") or ""))
        birth = st.text_input("تاريخ الميلاد", value=str(target.get("birth_date") or ""))
        area = st.text_input("المنطقة التأمينية", value=str(target.get("insurance_area") or ""))
        job = st.selectbox("الوظيفة", jobs, index=jobs.index(current_job) if current_job in jobs else 0)
        email = st.text_input("البريد الإلكتروني", value=str(target.get("email") or ""))
        recovery = st.text_input("البريد الاحتياطي", value=str(target.get("recovery_email") or ""))
        username = st.text_input("اسم المستخدم", value=str(target.get("username") or ""))
    ...
    if save:
        role_map = {
            ...
            "قيادي مركز رئيسى 1": "leader1",
            "قيادي مركز رئيسى 2": "leader2",
            "قيادي مركز رئيسى 3": "leader3",
        }
        for u in users:
            if u.get("id") == target.get("id"):
                u["display_name"] = name.strip()
                u["birth_date"] = birth.strip()
                u["insurance_area"] = area.strip()
                u["job_title"] = job
                u["role"] = role_map[job]
                u["email"] = email.strip()
                u["recovery_email"] = recovery.strip()
                u["username"] = username.strip()
                break
        save_users(users)
        try:
            _cached_users_snapshot.clear()
        except Exception:
            pass
        st.success("تم حفظ بيانات العضوية دون حذف القضايا أو المستندات.")
        st.rerun()

if st.session_state.page == "تعديل_بيانات_العضويات":
    edit_membership_data_page()
...
if st.session_state.page == "الرئيسية":
    ...
    def judicial_card(icon, title, page, key):
        # اسم واحد فقط: الزر نفسه هو بطاقة القسم ويفتح مباشرة عند الضغط عليه.
    ...
    section_specs = [
        ("⚖️", "تسجيل القضايا", "تسجيل", "card_add_v2"),
        ("📊", "الحصر العام", "الحصر", "card_list_v2"),
        ("🔔", "التنبيهات", "التنبيهات", "card_alerts_v2"),
        ("📑", "التقارير", "تقارير", "card_reports_v2"),
        ("🗂️", "الأرشيف", "الأرشيف", "card_archive_v2"),
        ("📚", "المكتبة القانونية", "مكتبة", "card_library_v2"),
        ("🔎", "البحث عن دعوى", "بحث", "card_search_v2"),
    ]
    for row_start in range(0, len(section_specs), 3):
        cols = st.columns(3)
        for col, spec in zip(cols, section_specs[row_start:row_start+3]):
            with col:
                judicial_card(*spec)

    # لوحة التحكم تظهر للادمن فقط، مرة واحدة، أسفل الأقسام.
    if is_admin:
        st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
        if st.button("⚙️  لوحة التحكم", use_container_width=True, key="admin_control_main_unique"):
            st.session_state.page = "لوحة التحكم"
            st.rerun()
  ...
  .stButton>button:hover {background: #B8860B!important; color: #000!important;}
    ...
    with tab1:
        ...
        if st.button("🔍 عرض بيان الدعاوى المتداولة", use_container_width=True, type="primary", key="show1"):
            ...
            with c1:
                st.download_button("🌐 HTML", data=html_export.encode("utf-8"), file_name=report_download_name("بيان_الدعاوى_المتداولة", عضو_قانوني1, lawyer, region, "html"), mime="text/html", use_container_width=True, key="html_dlx1")
            with c2:
                st.download_button("📄 استخراج PDF", data=to_pdf(pd.DataFrame(df_data), title, region, عضو_قانوني1, lawyer, مدير_ادارة1, مدير_عام1), file_name=report_download_name("بيان_الدعاوى_المتداولة", عضو_قانوني1, lawyer, region, "pdf"), mime="application/pdf", use_container_width=True, key="pdf_dlx1")
            with c3:
                st.download_button("📝 Word", data=to_word(pd.DataFrame(df_data), title, region, عضو_قانوني1, lawyer, مدير_ادارة1, مدير_عام1), file_name=report_download_name("بيان_الدعاوى_المتداولة", عضو_قانوني1, lawyer, region, "docx"), mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True, key="word_dlx1")
            with c4:
                ...
                with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:
                    ...
                    for col in ws.columns:
                        ...
                        ws.column_dimensions[letter].width = min(max(max_len + 4, 16), 55)
                st.download_button("⬇️ Excel", data=excel_buffer.getvalue(), file_name=report_download_name("بيان_الدعاوى_المتداولة", عضو_قانوني1, lawyer, region, "xlsx"), use_container_width=True, key="dlx1")
    # ========= تبويب 2: الاحكام =========
    with tab2:
        st.markdown("<div style='background:#1E2A47; padding:12px; border-radius:8px; border:2px solid #B8860B; margin-bottom:12px'>", unsafe_allow_html=True)
        ...
        if st.button("🔍 عرض بيان الاحكام", use_container_width=True, type="primary", key="show2"):
            ...
            c1,c2,c3,c4 = st.columns(4)
            with c1:
                st.download_button("🌐 HTML", data=html_export.encode("utf-8"), file_name=report_download_name("بيان_الاحكام", عضو_قانوني2, lawyer2, region2, "html"), mime="text/html", use_container_width=True, key="html_dlx2")
            with c2:
                st.download_button("📄 استخراج PDF", data=to_pdf(pd.DataFrame(df_data), title, region2, عضو_قانوني2, lawyer2, مدير_ادارة2, مدير_عام2), file_name=report_download_name("بيان_الاحكام", عضو_قانوني2, lawyer2, region2, "pdf"), mime="application/pdf", use_container_width=True, key="pdf_dlx2")
            with c3:
                st.download_button("📝 Word", data=to_word(pd.DataFrame(df_data), title, region2, عضو_قانوني2, lawyer2, مدير_ادارة2, مدير_عام2), file_name=report_download_name("بيان_الاحكام", عضو_قانوني2, lawyer2, region2, "docx"), mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True, key="word_dlx2")
            with c4:
                excel_buffer = io.BytesIO()
                with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:
                    df = pd.DataFrame(df_data)
                    ...
                    for col in ws.columns:
                        ...
                        max_len = max((len(str(cell.value or "")) for cell in col), default=10)
                        ws.column_dimensions[letter].width = min(max(max_len + 4, 16), 55)
                st.download_button("⬇️ Excel", data=excel_buffer.getvalue(), file_name=report_download_name("بيان_الاحكام", عضو_قانوني2, lawyer2, region2, "xlsx"), use_container_width=True, key="dlx2")
    # ================= القسم 2 من 2 =================
                # ========= تبويب 3: الاحصائيات =========
    with tab3:
    ...
    with tab4:
        ...
        if st.button("🔍 عرض البيان العددي", use_container_width=True, type="primary", key="show_stat"):
            ...
            c1,c2,c3,c4 = st.columns(4)
            with c1:
                st.download_button("🌐 HTML", data=html_export.encode("utf-8"), file_name=report_download_name("بيان_عددي", عضو_قانوني_stat, lawyer_stat, region_stat, "html"), mime="text/html", use_container_width=True, key="html_dl_stat3")
            with c2:
                st.download_button("📄 استخراج PDF", data=to_pdf(df_data, title_stat, region_stat, عضو_قانوني_stat, lawyer_stat, مدير_ادارة_stat, مدير_عام_stat), file_name=report_download_name("بيان_عددي", عضو_قانوني_stat, lawyer_stat, region_stat, "pdf"), mime="application/pdf", use_container_width=True, key="pdf_dl_stat3")
            with c3:
                st.download_button("📝 Word", data=to_word(df_data, title_stat, region_stat, عضو_قانوني_stat, lawyer_stat, مدير_ادارة_stat, مدير_عام_stat), file_name=report_download_name("بيان_عددي", عضو_قانوني_stat, lawyer_stat, region_stat, "docx"), mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True, key="word_dl_stat3")
            with c4:
                excel_buffer = io.BytesIO()
                with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:
                    df = df_data
                    ...
                    for col in ws.columns:
                        ...
                        max_len = max((len(str(cell.value or "")) for cell in col), default=10)
                        ws.column_dimensions[letter].width = min(max(max_len + 4, 16), 55)
                st.download_button("⬇️ Excel", data=excel_buffer.getvalue(), file_name=report_download_name("بيان_عددي", عضو_قانوني_stat, lawyer_stat, region_stat, "xlsx"), use_container_width=True, key="dl_stat3")
# ========================= نهاية الجزء 
import json
import os
...
def home_return_button(key):
    if st.button("↩️ العودة للصفحة الرئيسية", key=key, use_container_width=True):
        st.session_state.page = "الرئيسية"
        st.rerun()
