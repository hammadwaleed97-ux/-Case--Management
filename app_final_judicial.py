...
def site_management_page():
    ...
    st.markdown("<div class='admin-notice'>🔐 الأدمن منفصل تمامًا عن قضايا الأعضاء ولا يرى قضاياهم أو تقاريرهم.</div>", unsafe_allow_html=True)
    data = load_data(); users = [u for u in load_users() if u.get("role") == "member"]; banners = load_banners()
    unassigned = [c for c in data.get("cases", []) if not c.get("owner_id")]
    cards = [("👥","إدارة الأعضاء","الحسابات والحالات","ادارة_الاعضاء"),("🪪","استخراج عضوية","إنشاء حساب عضو جديد","extract_member"),("📢","إدارة اليافطات","نشر الإعلانات والتحكم في عددها","إدارة اليافطات"),("🗂️","توزيع القضايا القديمة",f"قضايا بلا صاحب: {len(unassigned)}","توزيع_القضايا")]
    cols=st.columns(2)
    ...
    c1,c2,c3=st.columns(3); c1.metric("الأعضاء",len(users)); c2.metric("اليافطات",len(banners)); c3.metric("قضايا غير مسندة",len(unassigned))

def distribute_legacy_cases_page():
    if not is_admin_user(): st.error("غير مصرح لك"); return
    st.markdown("<div class='section-ribbon'>🗂️ توزيع القضايا القديمة</div>",unsafe_allow_html=True)
    if st.button("⬅️ العودة لإدارة الموقع",use_container_width=True): st.session_state.page="إدارة الموقع"; st.rerun()
    data=load_data(); users=[u for u in load_users() if u.get("role")=="member" and u.get("status")=="active"]; unassigned=[c for c in data.get("cases",[]) if not c.get("owner_id")]
    if not unassigned: st.success("✅ كل القضايا الحالية مرتبطة بأعضاء."); return
    if not users: st.warning("لا يوجد أعضاء مفعلون لإسناد القضايا إليهم."); return
    names=[f"{u.get('username')} (ID: {u.get('id')})" for u in users]
    selected=st.selectbox("إسناد جميع القضايا القديمة غير المسندة إلى العضو",names,key="legacy_bulk_owner")
    if st.button("📌 إسناد الكل لهذا العضو",use_container_width=True,type="primary"):
        uid=users[names.index(selected)]["id"]
        for case in unassigned: case["owner_id"]=uid
        save_data(data); st.success(f"تم إسناد {len(unassigned)} قضية"); st.rerun()
    st.markdown("---"); st.subheader(f"القضايا غير المسندة: {len(unassigned)}")
    for idx,case in enumerate(unassigned):
        with st.container(border=True):
            st.write(f"**{case.get('رقم','-')} لسنة {case.get('سنة','-')} — {case.get('نوع','')}**")
            st.caption(f"{case.get('محكمة_اسم','')} | {case.get('موضوع','')}")
            choice=st.selectbox("العضو",names,key=f"legacy_owner_{case.get('id',idx)}")
            if st.button("إسناد القضية",key=f"assign_case_{case.get('id',idx)}",use_container_width=True):
                case["owner_id"]=users[names.index(choice)]["id"]; save_data(data); st.rerun()
...
.judicial-card{min-height:165px;background:linear-gradient(145deg,#111d33,#1f3150);border:1px solid rgba(212,175,55,.55);border-top:4px solid #D4AF37;border-radius:20px;padding:20px;margin:8px 0 4px;box-shadow:0 12px 30px rgba(0,0,0,.28);text-align:center;}
...
if is_admin_user() and st.session_state.page in CASE_PAGES:
    st.session_state.page = "إدارة الموقع"
...
elif st.session_state.page == "توزيع_القضايا":
    distribute_legacy_cases_page()
...
if st.session_state.page == "الرئيسية":
    if is_admin_user():
        st.markdown("<div class='section-ribbon'>⚙️ إدارة الموقع</div>",unsafe_allow_html=True)
        st.markdown("<div class='section-subtitle'>لوحة تحكم الأدمن — الحسابات واليافطات وإعدادات الموقع فقط</div>",unsafe_allow_html=True)
        show_banners()
        st.markdown("<div class='admin-notice'>🔐 حساب الأدمن منفصل عن قضايا الأعضاء. لا تظهر للأدمن أي قضية أو مكتبة أو تقارير.</div>",unsafe_allow_html=True)
        if st.button("⚙️ فتح إدارة الموقع",use_container_width=True,type="primary"):
            st.session_state.page="إدارة الموقع"; st.rerun()
        c1,c2,c3=st.columns(3)
        with c1:
            if st.button("🔑 تغيير كلمة السر",use_container_width=True): st.session_state.page="change_password"; st.rerun()
        with c2:
            if st.button("📧 إعداد البريد",use_container_width=True): st.session_state.page="recovery_settings"; st.rerun()
        with c3:
        ...
        st.markdown(f"<div class='section-subtitle'>مرحبًا {st.session_state.user.get('username','')} — هذه المساحة خاصة بقضايا حسابك فقط</div>",unsafe_allow_html=True)
        show_banners()
        ...
        for i,(icon,title,desc,target) in enumerate(cards):
            with cols[i%3]:
                ...
                if st.button(f"فتح {title}",key=f"member_card_{i}",use_container_width=True): st.session_state.page=target; st.rerun()
