...
def to_pdf(df, title, region="", member_name="", specialization="", manager_name="", general_manager_name="", *args, **kwargs):
    ...
    font_name = "ArabicReport" if font_path else "Helvetica"
    if font_path:
    ...
    if region:
        pdf.cell(0, 6, ar(f"منطقة: {region}"), 0, 1, "C")
    pdf.ln(3)

    # Table columns ordered right-to-left.
    if len(df.columns):
        pdf.set_font(font_name, "", 7)
        width = 280 / len(df.columns)
        for col in reversed(list(df.columns)):
            pdf.cell(width, 7, ar(col), 1, 0, "C")
        pdf.ln()
        for _, row in df.iterrows():
            for value in reversed(list(row)):
                pdf.cell(width, 7, ar(value), 1, 0, "C")
            pdf.ln()

    pdf.ln(5)
    pdf.set_font(font_name, "", 9)
    pdf.cell(0, 7, ar(f"تحريرًا في: {datetime.now().strftime('%d/%m/%Y')}"), 0, 1, "C")
    pdf.ln(3)
    pdf.set_font(font_name, "", 10)
    pdf.cell(0, 8, ar("وتفضلوا بقبول وافر الاحترام والتقدير"), 0, 1, "C")
    pdf.ln(8)

    # Same line: legal department manager on the left, legal member on the right.
    page_width = pdf.w
    box_w = 100
    left_x = 20
    ...
    pdf.set_xy(left_x, y)
    pdf.multi_cell(box_w, 7, ar("مدير الإدارة\n\n................................."), 0, "C")

    pdf.set_xy(right_x, y)
    member_text = "عضو الإدارة القانونية"
    if member_name:
        member_text += "\n" + str(member_name)
    pdf.multi_cell(box_w, 7, ar(member_text + "\n\n................................."), 0, "C")

    # General manager centered below both signatures.
    pdf.set_y(y + 35)
