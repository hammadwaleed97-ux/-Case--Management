...
def can_message(sender_role, target_role):
    ...
    if sender_role == "manager":
        # مدير الإدارة: إرسال للمدير العام، ومديري الإدارات، والأعضاء.
        return target_role in ("member", "manager", "general_manager")
    if sender_role == "leader1":
        return target_role in ("admin", "member", "manager", "general_manager", "leader1", "leader2", "leader3")
    if sender_role == "leader2":
        return target_role in ("member", "manager", "general_manager", "leader1", "leader2", "leader3")
    if sender_role == "leader3":
        return target_role in ("member", "manager", "general_manager", "leader2", "leader3")
...
def messaging_page():
    ...
    if target_kind == "محددون من أكثر من وظيفة":
        ...
        selected = st.multiselect(
            "اختر المستلمين",
            [f"{u.get('username')} — {role_label(u.get('role'))}" for u in candidates],
            key=f"msg_multi_{sr}"
        )
...
if st.session_state.page == "الرئيسية":
    ...
    if not is_admin:
        with b: judicial_card("🔐","حسابي","change_password","card_account_v2")

    st.markdown("---")
    a,b,c = st.columns([1,1.2,1])
    with b:
        if st.button("🚪 تسجيل الخروج", use_container_width=True, key="logout_main_v2"):
