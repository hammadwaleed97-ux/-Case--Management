...
# إعدادات البريد: تعريفها هنا قبل load_users و send_email
...
def load_users():
    ...
    default_users = [{"id":1,"username":ADMIN_USERNAME,"password":admin_pass,"email":SENDER_EMAIL,"recovery_email":"","role":"admin","status":"active","password_set":True}]
...
def login_page():
    ...
    <div class="login-shell">
      <h1 style="text-align:center;color:#D4AF37!important;">⚖️ إدارة القضايا ⚖️</h1>
      <h2 style="text-align:center;color:#fff!important;">دخول السادة الأعضاء</h2>
      <p style="text-align:center;color:#fff!important;">نظام إدارة القضايا والشئون القانونية</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if st.button("تفعيل الحساب", use_container_width=True, key="activate_account"):
            users = load_users()
            found_user = next((u for u in users if u.get("username") == member_name), None)
            if not found_user:
                st.error("الاسم ده مش موجود")
            elif found_user.get("status") in ("suspended_final", "deleted"):
                st.error("الحساب موقوف نهائياً ولا يمكن تفعيله")
...
def extract_member_page():
    ...
    with st.container(border=True):
        ...
        job_role = st.selectbox("الوظيفة", ["عضو قانونى", "مدير إدارة", "مدير عام"], key="new_member_job_role")
        ...
        if st.button("🪪 استخراج العضوية", use_container_width=True, type="primary", key="extract_member_new_v3"):
            ...
            if not name:
                ...
                if existing_user:
                    # لا نحذف أي بيانات سابقة؛ نحدث بيانات العضوية فقط.
                    existing_user["birth_date"] = birth_date.isoformat()
                    existing_user["insurance_area"] = area
                    existing_user["job_title"] = job_role
                    existing_user["role"] = {"عضو قانونى":"member", "مدير إدارة":"manager", "مدير عام":"general_manager"}.get(job_role, "member")
                    ...
                    users.append({
                        ...
                        "email": "",
                        "recovery_email": "",
                        "role": {"عضو قانونى":"member", "مدير إدارة":"manager", "مدير عام":"general_manager"}.get(job_role, "member"),
                        "job_title": job_role,
                        "status": "active",
                        "password_set": False,
...
def manage_users_page():
    ...
    for user in members:
        # توافق مع الأعضاء القدامى: الحقول الجديدة تضاف دون المساس بالبيانات السابقة.
        birth = user.get("birth_date")
        age, extended_active = member_age_status(user)
        ...
        with st.container(border=True):
            ...
            with c1:
                ...
                # تنبيه واضح بجانب بيانات العضو عند بلوغ الستين.
...
def role_label(role):
    return {"admin":"الادمن", "member":"عضو قانونى", "manager":"مدير إدارة", "general_manager":"مدير عام"}.get(role, role or "")

def can_message(sender_role, target_role):
    if sender_role == "admin":
        return target_role in ("member", "manager", "general_manager", "admin")
...
def messaging_page():
    ...
    sr = user.get("role")
    if sr not in ("admin", "manager", "general_manager"):
        return

    users = load_users()
    messages = load_messages()
    st.markdown("<h2 style=\"color:#D4AF37;text-align:center;\">📢 الرسائل والبيانات</h2>", unsafe_allow_html=True)
    st.caption("الرسائل محفوظة دائمًا ولا يؤدي إيقاف أو حذف حساب الدخول إلى حذف سجل الرسائل.")

    # صلاحيات الإرسال: الادمن ثابت وله الصلاحية الكاملة.
    if sr == "admin":
        choices = ["الجميع", "عضو محدد", "مدير إدارة محدد", "مدير عام محدد", "الادمن فقط"]
    elif sr == "manager":
        choices = ["الجميع", "عضو محدد", "مدير إدارة محدد", "مدير عام محدد", "الادمن فقط"]
    else:
        # المدير العام يمكنه اختيار الادمن صراحة وإرسال رسالة له، مع بقاء صلاحياته السابقة.
        choices = ["الادمن فقط", "عضو محدد", "مدير إدارة محدد", "مدير عام محدد"]

    target_kind = st.selectbox("إرسال إلى", choices, key=f"msg_target_kind_{sr}")
    text = st.text_area("نص الرسالة أو البيان", height=130, key=f"msg_text_{sr}")

    role_map = {
        "عضو محدد": "member",
        "مدير إدارة محدد": "manager",
        "مدير عام محدد": "general_manager",
        "الادمن فقط": "admin",
    }
    selected = []
    if target_kind in role_map:
        rr = role_map[target_kind]
        candidates = [u for u in users if u.get("status") == "active" and u.get("role") == rr]
        # في حالة المدير العام سيظهر اسم الادمن أمامه بوضوح.
        if rr == "admin" and sr == "general_manager":
            st.info("المستلم المتاح لك: الادمن فقط")
        selected = st.multiselect(
            "اختر الاسم",
            [u.get("username") for u in candidates],
            key=f"msg_selected_{sr}_{target_kind}"
        )
    ...
    if st.button("📤 إرسال الرسالة / البيان", type="primary", use_container_width=True, key=f"send_message_{sr}"):
        ...
        for u in users:
            ...
            if not can_message(sr, target_role):
                continue

            if target_kind == "الجميع":
                ok = True
            elif target_kind == "الادمن فقط":
                ok = target_role == "admin" and (not selected or u.get("username") in selected)
            else:
                ok = u.get("username") in selected

            if ok:
                recipients.append(u.get("id"))
...
def show_messages_for_user():
    """عرض الرسائل الواردة في أعلى الصفحة فور فتحها."""
    user = st.session_state.get("user") or {}
    uid = user.get("id")
    if not uid:
        return
    ...
    st.markdown("<div class=\"incoming-messages-title\">📬 الرسائل والبيانات الواردة</div>", unsafe_allow_html=True)
    for m in reversed(mine[-30:]):
        sender = html.escape(str(m.get("sender_name", "غير معروف")))
        sender_job = html.escape(str(m.get("sender_job_title") or role_label(m.get("sender_role")) or "غير محدد"))
        role = html.escape(str(role_label(m.get("sender_role")) or ""))
        created = html.escape(str(m.get("created_at", "")))
        message_text = html.escape(str(m.get("text", "")))
        st.markdown(
            f'<div style="background:linear-gradient(135deg,#14233d,#203b63);border:2px solid #D4AF37;border-radius:16px;padding:15px;margin:0 0 12px;box-shadow:0 8px 24px rgba(0,0,0,.25);">'
            f'<div style="color:#D4AF37;font-size:19px;font-weight:900;">📢 المرسل: {sender}</div>'
            f'<div style="color:#fff;font-size:16px;font-weight:800;margin-top:4px;">الوظيفة: {sender_job} — الصفة: {role}</div>'
            f'<div style="color:#fff;font-size:17px;font-weight:700;margin-top:10px;white-space:pre-wrap;">{message_text}</div>'
            f'<div style="color:#cbd5e1;font-size:12px;margin-top:8px;">{created}</div></div>',
            unsafe_allow_html=True
        )
...
def global_people_search():
    ...
    if q.strip():
        matches=[u for u in users if q.strip().lower() in str(u.get("username","")).lower()]
        if matches:
            for u in matches:
                st.markdown(f"**👤 {u.get('username')}** — {role_label(u.get('role'))} — {u.get('insurance_area') or 'لا توجد منطقة'} — الحالة: {u.get('status','')}")
        else: st.info("لا توجد نتيجة بهذا الاسم")

def admin_control_page():
    """لوحة تحكم موحدة للادمن فقط."""
    if not st.session_state.user or st.session_state.user.get("role") != "admin":
    ...
    tab_users, tab_messages, tab_account = st.tabs(["👥 الأعضاء", "📢 الرسائل والبيانات", "🔐 حساب الادمن والإيميل"])
    ...
    with tab_messages:
        ...
        with st.form("admin_banner_add_v2"):
            ...
            if audience == "أعضاء محددين":
                visible_to = st.multiselect("حدد المستلمين", [u.get("username") for u in users if u.get("status") == "active"], key="banner_users_v2")
        ...
        # لا نحذف المنتهية تلقائياً حتى لا نفقد البيانات؛ فقط لا نعرضها للأعضاء.
            ...
            with st.container(border=True):
                ...
                if st.session_state.get("edit_banner_id") == b.get("id"):
                    with st.form(f"edit_banner_form_{b.get('id',i)}"):
                        ...
                        nvis = []
                        if na == "أعضاء محددين":
                            nvis = st.multiselect("المستلمون", [u.get("username") for u in users if u.get("status") == "active"], default=b.get("visible_to",[]), key=f"ev_{b.get('id',i)}")
                        ...
                        with ec1:
                            if st.form_submit_button("💾 حفظ التعديل", use_container_width=True):
                                ...
                                b["visible_to"] = nvis
                                # نحافظ على الوقت إن أمكن، ونغير التاريخ فقط
...
def show_banners():
    ...
    for banner in banners:
        if datetime.fromisoformat(banner['expire_at']) > now:
            ...
            if banner.get('image_data'):
                st.image(banner['image_data'], use_container_width=True)
            elif banner.get('image_path') and os.path.exists(banner['image_path']):
...
if st.session_state.page == "login":
    ...
    with st.expander("➕ اضافة يافطة جديدة"):
        ...
        st.markdown("<p style='color:white; font-weight:bold; font-size:16px;'>🎨 لون الرسالة أو البيان</p>", unsafe_allow_html=True)
            ...
            if title and content:
                ...
                if uploaded_file:
                    ...
                    with open(image_path, "wb") as f:
                        f.write(raw_image)
                    image_data = "data:" + (uploaded_file.type or "image/png") + ";base64," + base64.b64encode(raw_image).decode("utf-8")
    ...
    # اليافطات تظهر أولاً، ثم لوحة التحكم، ثم الأقسام
    show_banners()
    show_messages_for_user()
    ...
    if is_admin:
        ...
        <div class="admin-control-top">
            <div style="text-align:center;color:#D4AF37;font-size:25px;font-weight:900;">⚙️ لوحة التحكم</div>
            <div style="text-align:center;color:#fff;margin-top:4px;">إدارة الموقع والأعضاء واليافطات — للادمن فقط</div>
        </div>
        """, unsafe_allow_html=True)
        if st.button("⚙️ لوحة التحكم", use_container_width=True, type="primary", key="open_admin_control_top"):
            st.session_state.page = "لوحة التحكم"
            st.rerun()
    ...
    def judicial_card(icon, title, page, key):
        st.markdown(
            f"<div class='judicial-card'><div class='judicial-card-icon'>{icon}</div>"
            f"<div class='judicial-card-title'>{title}</div></div>",
            unsafe_allow_html=True
        )
    ...
    # توزيع غير متماثل: يمين / وسط / شمال / أعلى / أسفل
    a,b,c = st.columns([1,1.2,1])
    with a: judicial_card("⚖️","تسجيل القضايا","تسجيل","card_add")
    with b: judicial_card("📋","الحصر العام","الحصر","card_list")
    with c: judicial_card("🔍","البحث عن دعوى","بحث","card_search")

    a,b = st.columns(2)
    with a: judicial_card("📊","التقارير","تقارير","card_reports")
    with b: judicial_card("📚","المكتبة القانونية","مكتبة","card_library")

    a,b,c = st.columns([1,1.2,1])
    with b: judicial_card("🔴","مركز التنبيهات","التنبيهات","card_alerts")

    if user.get("role") in ("admin", "manager", "general_manager"):
        a,b,c = st.columns([1,1.2,1])
        with b: judicial_card("📢","الرسائل والبيانات","الرسائل","card_messages")

    a,b = st.columns(2)
    with a: judicial_card("🗄️","الأرشيف","الأرشيف","card_archive")
    if not is_admin:
        with b: judicial_card("🔐","حسابي","change_password","card_account")
    else:
        with b: judicial_card("📁","الملف الشخصي","الحصر","card_profile_admin")

    st.markdown("---")
    a,b,c = st.columns([1,1.2,1])
    with b:
        if st.button("🚪 تسجيل الخروج", use_container_width=True, key="logout_main"):
            st.session_state.user = None
...
# تم تعريف إعدادات البريد في بداية الملف
...
# ================================================
    ...
    if not data["cases"]:
        ...
        active_cases = [c for c in data["cases"] if c.get('حالة') == 'متداولة' and c.get("assigned_to") == (st.session_state.user or {}).get("username")]
        # ==================================================
...
# ==============================================
    ...
    قضايا_منتهية = [c for c in data["cases"] if c.get("حالة") == "منتهية" and c.get("assigned_to") == (st.session_state.user or {}).get("username")]
...
if st.session_state.page == "تقارير":
    ...
    from datetime import datetime
    data = load_data()
    current_username = st.session_state.user.get("username", "") if st.session_state.get("user") else ""
    my_cases = [c for c in data.get("cases", []) if c.get("assigned_to") == current_username]
...
# هذه الصفحة هي المدخل الوحيد لأدوات الادمن من الواجهة الرئيسية.
if st.session_state.get("page") == "لوحة التحكم":
    ...
    if st.session_state.get("user", {}).get("role") in ("admin", "manager", "general_manager"):
        messaging_page()
    else:
        st.session_state.page = "الرئيسية"
        st.rerun()
