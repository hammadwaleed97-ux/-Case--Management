...
import json, os, bcrypt, smtplib, random, io, requests, base64, secrets
...
# إعدادات البريد: تُقرأ من Streamlit Secrets أو متغيرات البيئة
...
def send_email(to_email, subject, body):
    if not SENDER_EMAIL or not SENDER_PASSWORD:
        st.error("بيانات البريد غير موجودة في Streamlit Secrets")
        return False
    try:
...
def _make_default_admin():
    """إنشاء حساب الأدمن الافتراضي بشكل آمن حتى لو كانت بعض الإعدادات غير موجودة."""
    username = str(globals().get("ADMIN_USERNAME") or "admin")
    default_pass = str(globals().get("ADMIN_DEFAULT_PASS") or "admin123")
    sender_email = str(globals().get("SENDER_EMAIL") or "")
    password_hash = bcrypt.hashpw(default_pass.encode("utf-8"), bcrypt.gensalt()).decode("utf-8")
    return {
        "id": 1,
        "username": username,
...
def load_users():
    # إنشاء القائمة الافتراضية داخل الدالة بعد تعريف الإعدادات.
    default_users = [_make_default_admin()]
    users = load_json_persistent("users", USERS_FILE, default_users)

    if not isinstance(users, list) or not users:
        users = default_users
        save_json_persistent("users", USERS_FILE, users)

    # ضمان الحقول الأساسية حتى لا تتعطل البيانات القديمة.
    for user in users:
        user.setdefault("id", 0)
        user.setdefault("username", "")
        user.setdefault("password", "")
        user.setdefault("email", "")
        user.setdefault("recovery_email", "")
        user.setdefault("role", "member")
        user.setdefault("status", "active")
        user.setdefault("password_set", bool(user.get("password")))
...
def check_login(username, password):
    ...
    for user in users:
        if user.get("username") == username and user.get("status") == "active":
...
def login_page():
    st.markdown("""
    <style>
    .stApp{background:radial-gradient(circle at top,#243A5E 0%,#0A1428 48%,#050A13 100%)!important;}
    .login-shell{max-width:720px;margin:30px auto 10px;padding:34px 30px;border:1px solid #D4AF37;border-radius:28px;background:linear-gradient(145deg,rgba(17,29,51,.98),rgba(7,13,25,.98));box-shadow:0 25px 70px rgba(0,0,0,.55),inset 0 0 30px rgba(212,175,55,.08);text-align:center;}
    .login-emblem{font-size:58px;color:#D4AF37;text-shadow:0 0 18px rgba(212,175,55,.35);}
    .login-title{font-size:34px;font-weight:900;color:#D4AF37;margin:4px 0;}
    .login-subtitle{color:#F4E6B0;font-size:15px;font-weight:700;margin-bottom:22px;}
    .login-label{color:#FFF!important;font-weight:900!important;font-size:16px;margin:12px 0 6px;text-align:right;}
    .stTextInput input{background:#F8FAFC!important;color:#111827!important;border:2px solid #D4AF37!important;border-radius:12px!important;font-weight:700!important;}
    div[data-testid="stTabs"] button{color:#FFF!important;font-weight:900!important;font-size:16px!important;}
    div[data-testid="stTabs"] button[aria-selected="true"]{color:#D4AF37!important;}
    .stButton>button{background:linear-gradient(135deg,#D4AF37,#F4D77A,#B8860B)!important;color:#101828!important;border:1px solid #FFE9A6!important;box-shadow:0 7px 18px rgba(212,175,55,.22)!important;}
    </style>
    <div class="login-shell"><div class="login-emblem">⚖️</div><div class="login-title">دخول السادة الأعضاء</div><div class="login-subtitle">منظومة إدارة القضايا والشئون القانونية</div></div>
    ...
    with tab1:
        ...
        password = st.text_input("كلمة السر", type="password", key="login_pass", label_visibility="collapsed")
        ...
        admin_recover_email = st.text_input("إيميل استرجاع الادمن", key="admin_recover", label_visibility="collapsed")
        ...
        member_recover_email = st.text_input("إيميل العضو", key="member_recover", label_visibility="collapsed")
        ...
        if st.session_state.get("show_reset_admin") or st.session_state.get("show_reset_member"):
            ...
            code_input = st.text_input("كود الاسترجاع", key="code_input", label_visibility="collapsed")

            st.markdown("<p style='color:white; font-weight:bold;'>كلمة السر الجديدة</p>", unsafe_allow_html=True)
            new_pass = st.text_input("كلمة السر الجديدة", type="password", key="new_pass", label_visibility="collapsed")
...
def extract_member_page():
    ...
    with st.container(border=True):
        ...
        new_username = st.text_input("اسم المستخدم الجديد", key="new_username", label_visibility="collapsed")
...
BANNERS_FILE = os.path.join(LOCAL_DATA_DIR, "banners_v2.json")
SITE_SETTINGS_FILE = os.path.join(LOCAL_DATA_DIR, "site_settings.json")
...
def save_banners(banners):
    return save_json_persistent("banners", BANNERS_FILE, banners)

def load_site_settings():
    defaults = {"banner_mode": "all", "banner_count": 3}
    settings = load_json_persistent("site_settings", SITE_SETTINGS_FILE, defaults)
    if not isinstance(settings, dict):
        settings = defaults.copy()
    settings.setdefault("banner_mode", "all")
    settings.setdefault("banner_count", 3)
    try: settings["banner_count"] = max(1, int(settings["banner_count"]))
    except Exception: settings["banner_count"] = 3
    return settings

def save_site_settings(settings):
    return save_json_persistent("site_settings", SITE_SETTINGS_FILE, settings)

def _banner_expire_datetime(banner):
    raw = banner.get("expire_at") or banner.get("expire")
    if not raw: return datetime.max
    try:
        value = str(raw)
        return datetime.fromisoformat(value + "T23:59:59" if len(value) == 10 else value)
...
def show_banners():
    ...
    active=[]
    for b in banners:
        if _banner_expire_datetime(b) < now: continue
        audience = b.get("audience", "all")
        ids = [str(x) for x in b.get("user_ids", [])]
        if audience == "selected" and str(uid) not in ids and not is_admin_user():
            continue
        active.append(b)
    settings = load_site_settings()
    if settings.get("banner_mode") == "count":
        active = active[:max(1, int(settings.get("banner_count", 3)))]
    for banner in active:
        color = banner.get("color", "#D4AF37")
        font_size = int(banner.get("font_size", 18) or 18)
        st.markdown(f"<div class='judicial-banner' style='background:{color};font-size:{font_size}px;'>📢 {banner.get('text','')}</div>", unsafe_allow_html=True)
        if banner.get("image_data"): st.image(banner["image_data"], use_container_width=True)
        elif banner.get("image_path") and os.path.exists(banner["image_path"]): st.image(banner["image_path"], use_container_width=True)
...
def is_admin_user():
    return bool(st.session_state.get("user") and st.session_state.user.get("role") == "admin")

def current_user_id():
    try: return int(st.session_state.user.get("id"))
    except Exception: return None

def visible_cases(data):
    uid = current_user_id()
    if uid is None: return []
    return [c for c in data.get("cases", []) if str(c.get("owner_id")) == str(uid)]

def get_owned_case(data, case_id):
    uid = current_user_id()
    if uid is None: return None
    return next((c for c in data.get("cases", []) if c.get("id") == case_id and str(c.get("owner_id")) == str(uid)), None)

def site_management_page():
    if not is_admin_user(): st.error("غير مصرح لك"); return
    st.markdown("<div class='section-ribbon'>⚙️ إدارة الموقع</div>", unsafe_allow_html=True)
    st.markdown("<div class='section-subtitle'>الأدمن فقط — إدارة الحسابات واليافطات وإعدادات الموقع</div>", unsafe_allow_html=True)
    st.markdown("<div class='admin-notice'>🔐 لوحة إدارة الموقع مستقلة في الأعلى، والأدمن يحتفظ أيضًا بملف قضاياه الشخصي كعضو مستقل.</div>", unsafe_allow_html=True)
    users = [u for u in load_users() if u.get("role") == "member"]; banners = load_banners()
    cards = [("👥","إدارة الأعضاء","الحسابات والحالات","ادارة_الاعضاء"),("🪪","استخراج عضوية","إنشاء حساب عضو جديد","extract_member"),("📢","إدارة اليافطات","نشر الإعلانات والتحكم في الظهور","إدارة اليافطات")]
    cols=st.columns([1,2,1])
    for i,(icon,title,desc,target) in enumerate(cards):
        with cols[i%3]:
            st.markdown(f"<div class='judicial-card admin-card'><div class='card-icon'>{icon}</div><div class='card-title'>{title}</div><div class='card-desc'>{desc}</div></div>",unsafe_allow_html=True)
            if st.button(f"فتح {title}",key=f"admin_card_{i}",use_container_width=True,type="primary"):
                st.session_state.page=target; st.rerun()
    st.markdown("<div class='section-ribbon small'>📊 ملخص إدارة الموقع</div>",unsafe_allow_html=True)
    c1,c2=st.columns(2); c1.metric("الأعضاء",len(users)); c2.metric("اليافطات",len(banners))
...
.section-ribbon{background:linear-gradient(135deg,#111E35,#243A5E);border:1px solid #D4AF37;border-right:6px solid #D4AF37;border-radius:18px;padding:16px 20px;margin:18px 0 8px;color:#D4AF37;font-size:25px;font-weight:900;text-align:right;box-shadow:0 10px 28px rgba(0,0,0,.25);}
.section-ribbon.small{font-size:20px;margin-top:28px;}
.section-subtitle{text-align:center;color:#D9E2F3;font-size:15px;font-weight:700;margin:0 0 18px;}
.admin-notice{background:linear-gradient(135deg,#2b1f08,#47340d);border:1px solid #D4AF37;color:#FFE9A6;border-radius:14px;padding:13px 16px;text-align:center;font-weight:800;margin-bottom:18px;}
.control-panel{background:linear-gradient(135deg,#07101f,#172a4b 55%,#07101f);border:2px solid #D4AF37;border-radius:22px;padding:18px;text-align:center;color:#D4AF37;font-size:28px;font-weight:900;box-shadow:0 12px 35px rgba(0,0,0,.38),inset 0 0 20px rgba(212,175,55,.08);margin:4px 0 14px;}
.mini-admin-card{min-height:92px;background:linear-gradient(145deg,#101d34,#1c3152);border:1px solid rgba(212,175,55,.6);border-radius:18px;padding:12px;text-align:center;color:#fff;box-shadow:0 8px 20px rgba(0,0,0,.25);}
.mini-admin-card b{display:block;color:#D4AF37;font-size:16px;font-weight:900;margin-top:4px;}
.mini-admin-card small{display:block;color:#DCE6F4;font-size:11px;margin-top:2px;}
.mini-icon{font-size:28px;}
.judicial-card{min-height:165px;background:linear-gradient(145deg,#111d33,#1f3150);border:1px solid rgba(212,175,55,.55);border-top:4px solid #D4AF37;border-radius:20px;padding:20px;margin:8px 0 4px;box-shadow:0 12px 30px rgba(0,0,0,.28),inset 0 0 18px rgba(212,175,55,.035);text-align:center;transition:transform .2s,box-shadow .2s;}
.judicial-card:hover{transform:translateY(-3px);box-shadow:0 18px 38px rgba(0,0,0,.35),0 0 20px rgba(212,175,55,.10);}

.admin-card{min-height:145px;}
.card-icon{font-size:38px;line-height:1.1;margin-bottom:8px;}
.card-title{color:#D4AF37;font-size:20px;font-weight:900;margin-bottom:7px;}
.card-desc{color:#DDE6F4;font-size:13px;font-weight:600;line-height:1.7;}
.judicial-banner{color:#101010;padding:14px 18px;border-radius:14px;margin:9px 0;font-weight:900;text-align:center;box-shadow:0 8px 18px rgba(0,0,0,.22);border:1px solid rgba(255,255,255,.25);}
...
label,.stTextInput label,.stSelectbox label,.stMultiSelect label,.stDateInput label,.stNumberInput label,.stTextArea label,.stRadio label{color:#FFF!important;font-weight:900!important;}
[data-testid="stWidgetLabel"] p{color:#FFF!important;font-weight:900!important;}
.stMarkdown,.stCaption{color:#F2F5FA!important;}
...
CASE_PAGES = {"تسجيل", "الحصر", "تفاصيل", "الأرشيف", "بحث", "التنبيهات", "مكتبة", "تقارير"}
...
if st.session_state.page == "extract_member":
    if st.session_state.user.get("role") == "admin":
        extract_member_page()
    else:
        st.error("غير مصرح لك")

elif st.session_state.page == "إدارة الموقع":
    site_management_page()

elif st.session_state.page == "ادارة_الاعضاء":
    if st.session_state.user.get("role") == "admin": manage_users_page()
    else: st.error("غير مصرح لك")
...
if st.session_state.page == "الرئيسية":
    ...
    if is_admin_user():
        st.markdown("<div class='control-panel'>⚙️ لوحة تحكم الأدمن</div>", unsafe_allow_html=True)
        st.markdown("<div class='section-subtitle'>إدارة الموقع أعلى الصفحة — والأدمن عضو مستقل له ملف قضاياه الخاص مثل أي عضو.</div>", unsafe_allow_html=True)
        adm_cols = st.columns([1, 1, 1, 1])
        admin_actions = [("⚙️", "إدارة الموقع", "إدارة الأعضاء واليافطات", "إدارة الموقع"), ("🔑", "كلمة السر", "تغيير كلمة المرور", "change_password"), ("📧", "البريد", "إعداد البريد والاسترداد", "recovery_settings"), ("🚪", "خروج", "تسجيل الخروج", "logout")]
        for i, (icon, title, desc, target) in enumerate(admin_actions):
            with adm_cols[i]:
                st.markdown(f"<div class='mini-admin-card'><div class='mini-icon'>{icon}</div><b>{title}</b><small>{desc}</small></div>", unsafe_allow_html=True)
                if st.button(title, key=f"admin_top_{i}", use_container_width=True):
                    if target == "logout": st.session_state.user=None; st.session_state.page="login"
                    else: st.session_state.page=target
                    st.rerun()

    st.markdown("<div class='section-ribbon'>⚖️ مكتب القضايا</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='section-subtitle'>مرحبًا {user_name} — هذه المساحة خاصة بقضايا حسابك فقط</div>", unsafe_allow_html=True)
    show_banners()
    cards=[("⚖️","تسجيل القضايا","إضافة قضية جديدة إلى ملفاتك","تسجيل"),("📋","الحصر العام","متابعة القضايا المتداولة والجلسات","الحصر"),("🔴","مركز التنبيهات","جلسات وطعون ومواعيد قريبة","التنبيهات"),("📊","التقارير","استخراج تقارير قضايا حسابك","تقارير"),("📚","المكتبة القانونية","موادك ومستنداتك القانونية","مكتبة"),("🗄️","الأرشيف","الأحكام والقضايا المحفوظة","الأرشيف"),("🔍","البحث عن دعوى","بحث داخل قضاياك فقط","بحث")]
    patterns = [[1,2,1],[2,1],[1,1,1],[1,2,1]]
    idx=0
    for row_no, widths in enumerate(patterns):
        cols=st.columns(widths)
        for col in cols:
            if idx >= len(cards): break
            icon,title,desc,target=cards[idx]
            with col:
                st.markdown(f"<div class='judicial-card'><div class='card-icon'>{icon}</div><div class='card-title'>{title}</div><div class='card-desc'>{desc}</div></div>",unsafe_allow_html=True)
                if st.button(f"فتح {title}",key=f"member_card_{idx}",use_container_width=True): st.session_state.page=target; st.rerun()
            idx += 1
    st.markdown("<div class='section-ribbon small'>👤 حسابي</div>",unsafe_allow_html=True)
    c1,c2,c3=st.columns(3)
    with c1:
        if st.button("🔑 تغيير كلمة السر",use_container_width=True): st.session_state.page="change_password"; st.rerun()
    with c2:
        if st.button("📧 تأكيد البريد",use_container_width=True): st.session_state.page="recovery_settings"; st.rerun()
    with c3:
        if st.button("🚪 تسجيل الخروج",use_container_width=True): st.session_state.user=None; st.session_state.page="login"; st.rerun()
...
# ====== الجزء الثاني: تسجيل القضية ============
    ...
    نوع = st.selectbox("نوع القضية", ["دعوى", "استئناف", "طعن"], key="case_type_add", label_visibility="collapsed")
    ...
    with st.form("form_case_add", clear_on_submit=True):  # <-- دي بتفضي الفورم لوحدها
        ...
        if st.form_submit_button("💾 حفظ القضية", use_container_width=True, type="primary"):
            if not رقم or not سنة: 
                ...
                new_case = {
                    "id": len(data["cases"])+1, "owner_id": current_user_id(), "نوع": نوع, "محكمة_اسم": محكمة_اسم, "مأمورية": مأمورية, 
...
# ================================================
    ...
    user_cases = visible_cases(data)
    if not user_cases:
        st.info("لا توجد قضايا مسجلة في حسابك")
    else:
        ...
        user_cases = visible_cases(data)
        active_cases = [c for c in user_cases if c.get('حالة') == 'متداولة']
        # ==================================================
...
# ================================================
    ...
    data = load_data()
    case = get_owned_case(data, st.session_state.selected_case_id)
    if not case:
        st.error("القضية غير موجودة أو ليست ضمن قضايا حسابك")
...
# ==============================================
    ...
    قضايا_منتهية = [c for c in user_cases if c.get("حالة") == "منتهية"]
...
# ==================================================
    ...
    if st.session_state.user["role"] == "admin":
        st.error("الأدمن مخصص لإدارة الموقع فقط")
        st.stop()
  ...
  .stButton>button:hover {background: #B8860B!important; color: #000!important;}
    ...
    with tab1:
        ...
        if st.button("🔍 عرض بيان الدعاوى المتداولة", use_container_width=True, type="primary", key="show1"):
            cases = [c for c in user_cases if str(c.get('حالة','')).strip() == 'متداولة']
            فلترة_بالمدة = []
            for c in cases:
                if c.get('تاريخ_جلسة'):
                    try:
    ...
    with tab2:
        ...
        if st.button("🔍 عرض بيان الاحكام", use_container_width=True, type="primary", key="show2"):
            archive = [c for c in user_cases if c.get("حالة") == "منتهية" and not c.get("تم_الحفظ_النهائي")]
            cases = []
            for c in archive:
...
if st.session_state.page == "إدارة اليافطات":
    ...
    st.markdown("<div class='section-ribbon'>📢 إدارة اليافطات</div>", unsafe_allow_html=True)
    st.markdown("<div class='section-subtitle'>تحكم كامل في النص، مدة الظهور، وعدد اليافطات والجمهور المستهدف.</div>", unsafe_allow_html=True)
    users = [u for u in load_users() if u.get("role") == "member" and u.get("status") == "active"]
    user_options = {f"{u.get('username','')} — ID {u.get('id')}": str(u.get('id')) for u in users}

    settings = load_site_settings()
    with st.container(border=True):
        st.markdown("### 🎛️ إعدادات الظهور العامة")
        c1,c2=st.columns(2)
        with c1:
            mode_label = st.selectbox("عدد اليافطات الظاهرة للأعضاء", ["إظهار الكل", "إظهار عدد معين"], index=0 if settings.get("banner_mode","all")=="all" else 1, key="banner_mode_admin")
        with c2:
            count = st.number_input("العدد عند اختيار عدد معين", min_value=1, max_value=100, value=int(settings.get("banner_count",3)), step=1, key="banner_count_admin")
        if st.button("💾 حفظ إعدادات الظهور", use_container_width=True, type="primary"):
            settings["banner_mode"] = "all" if mode_label == "إظهار الكل" else "count"
            settings["banner_count"] = int(count)
            save_site_settings(settings); st.success("تم حفظ إعدادات الظهور"); st.rerun()

    with st.container(border=True):
        st.markdown("### ➕ إضافة يافطة جديدة")
        with st.form("add_banner_form_v2"):
            text = st.text_area("نص اليافطة", placeholder="اكتب الإعلان أو التنبيه هنا...", height=100)
            c1,c2,c3=st.columns(3)
            with c1: color=st.color_picker("لون اليافطة", "#D4AF37")
            with c2: expire=st.date_input("تاريخ انتهاء الظهور", value=datetime.now().date()+timedelta(days=7), min_value=datetime.now().date())
            with c3: font_size=st.number_input("حجم الخط", min_value=12, max_value=40, value=18, step=1)
            audience_label=st.radio("نطاق الظهور", ["كل الأعضاء", "أعضاء محددون"], horizontal=True)
            selected_labels=st.multiselect("اختر الأعضاء", list(user_options.keys()), disabled=audience_label=="كل الأعضاء")
            publish=st.form_submit_button("🚀 نشر اليافطة", use_container_width=True)
            if publish:
                if not text.strip(): st.error("اكتب نص اليافطة أولًا")
                elif audience_label=="أعضاء محددون" and not selected_labels: st.error("اختر عضوًا واحدًا على الأقل")
                else:
                    banners=load_banners()
                    banners.append({"id":secrets.token_hex(8),"text":text.strip(),"color":color,"font_size":int(font_size),"expire":str(expire),"created_at":datetime.now().strftime("%Y-%m-%d %H:%M"),"audience":"all" if audience_label=="كل الأعضاء" else "selected","user_ids":[] if audience_label=="كل الأعضاء" else [user_options[x] for x in selected_labels]})
                    save_banners(banners); st.success("تم نشر اليافطة بنجاح"); st.rerun()

    banners=load_banners()
    st.markdown(f"<div class='section-ribbon small'>📋 اليافطات الحالية — {len(banners)}</div>", unsafe_allow_html=True)
    if not banners:
        st.info("لا توجد يافطات حاليًا.")
    else:
        for i,banner in enumerate(banners):
            expire_raw=banner.get("expire") or banner.get("expire_at") or ""
            try: remaining=(datetime.fromisoformat(str(expire_raw))-datetime.now()).days
            except Exception: remaining=None
            audience="كل الأعضاء" if banner.get("audience","all")=="all" else "أعضاء محددون"
            target_names=[]
            for uid in banner.get("user_ids",[]):
                u=next((x for x in users if str(x.get("id"))==str(uid)),None)
                if u: target_names.append(u.get("username",""))
            with st.container(border=True):
                st.markdown(f"<div class='judicial-banner' style='background:{banner.get('color','#D4AF37')};font-size:{int(banner.get('font_size',18))}px;'>📢 {banner.get('text','')}</div>", unsafe_allow_html=True)
                st.caption(f"الجمهور: {audience} | الانتهاء: {expire_raw or 'غير محدد'} | المتبقي: {remaining if remaining is not None else 'غير محدد'} يوم")
                if target_names: st.caption("المستهدفون: " + "، ".join(target_names))
                e1,e2=st.columns(2)
                with e1:
                    with st.expander("✏️ تعديل اليافطة", expanded=False):
                        with st.form(f"edit_banner_{i}"):
                            etext=st.text_area("النص", value=banner.get("text",""), height=90, key=f"etext_{i}")
                            ec1,ec2,ec3=st.columns(3)
                            with ec1: ecolor=st.color_picker("اللون", banner.get("color","#D4AF37"), key=f"ecolor_{i}")
                            with ec2:
                                try: default_date=datetime.fromisoformat(str(expire_raw)).date()
                                except Exception: default_date=datetime.now().date()+timedelta(days=7)
                                edate=st.date_input("تاريخ الانتهاء", value=default_date, key=f"edate_{i}")
                            with ec3: efsize=st.number_input("حجم الخط",12,40,int(banner.get("font_size",18)),1,key=f"efsize_{i}")
                            eaud=st.radio("نطاق الظهور",["كل الأعضاء","أعضاء محددون"],index=0 if banner.get("audience","all")=="all" else 1,horizontal=True,key=f"eaud_{i}")
                            old_selected=[k for k,v in user_options.items() if str(v) in [str(x) for x in banner.get("user_ids",[])]]
                            esel=st.multiselect("الأعضاء المحددون",list(user_options.keys()),default=old_selected,disabled=eaud=="كل الأعضاء",key=f"esel_{i}")
                            save_edit=st.form_submit_button("💾 حفظ التعديل",use_container_width=True)
                            if save_edit:
                                if not etext.strip(): st.error("النص لا يمكن أن يكون فارغًا")
                                elif eaud=="أعضاء محددون" and not esel: st.error("اختر عضوًا واحدًا على الأقل")
                                else:
                                    banner["text"]=etext.strip(); banner["color"]=ecolor; banner["font_size"]=int(efsize); banner["expire"]=str(edate); banner["audience"]="all" if eaud=="كل الأعضاء" else "selected"; banner["user_ids"]=[] if eaud=="كل الأعضاء" else [user_options[x] for x in esel]
                                    save_banners(banners); st.success("تم تعديل اليافطة"); st.rerun()
                with e2:
                    if st.button("🗑️ حذف اليافطة", key=f"delete_banner_v2_{i}", use_container_width=True):
                        banners.pop(i); save_banners(banners); st.success("تم حذف اليافطة"); st.rerun()
