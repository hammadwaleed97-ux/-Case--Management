...
def to_word(df, title, region, member_name="", specialization=""):
    doc = Document()
    ...
    if region:
        rtl(f"ديوان عام منطقة {region}", 12, True, WD_ALIGN_PARAGRAPH.CENTER)
    rtl(title, 13, True, WD_ALIGN_PARAGRAPH.CENTER)
    if member_name:
        rtl(f"عضو الإدارة القانونية: {member_name}", 11, True)
    if specialization:
        rtl(f"اختصاص الأستاذ: {specialization}", 11, True)
    ...
    rtl(f"تحرر في: {datetime.now().strftime('%d-%m-%Y')}", 10)
    output = io.BytesIO()
    doc.save(output)
    return output.getvalue()
...
def to_pdf(df, title, region, member_name="", specialization=""):
    ...
    pdf.add_page()
    font = _report_font_path()
    if font:
        pdf.add_font("ArabicReport", "", font, uni=True)
        pdf.set_font("ArabicReport", "", 15)
    else:
        pdf.set_font("Helvetica", "", 11)

    def ar(value):
        return fix_arabic("" if value is None else str(value))

    pdf.cell(0, 9, ar("الهيئة القومية للتأمين الاجتماعي"), 0, 1, "C")
    pdf.set_font("ArabicReport" if font else "Helvetica", "", 11)
    pdf.cell(0, 7, ar("الإدارة المركزية للإدارات القانونية"), 0, 1, "C")
    pdf.cell(0, 7, ar("الإدارة العامة للقضايا"), 0, 1, "C")
    if region:
        pdf.cell(0, 7, ar(f"ديوان عام منطقة {region}"), 0, 1, "C")
    pdf.cell(0, 8, ar(title), 0, 1, "C")
    if member_name:
        pdf.cell(0, 7, ar(f"عضو الإدارة القانونية: {member_name}"), 0, 1, "R")
    if specialization:
        pdf.cell(0, 7, ar(f"اختصاص الأستاذ: {specialization}"), 0, 1, "R")

    pdf.ln(3)
    pdf.set_font("ArabicReport" if font else "Helvetica", "", 7)
    if len(df.columns):
  ...
  .stButton>button:hover {background: #B8860B!important; color: #000!important;}
    ...
    with tab1:
        ...
        if st.button("🔍 عرض بيان الدعاوى المتداولة", use_container_width=True, type="primary", key="show1"):
            ...
            if not cases: st.warning(f"⚠️ لا توجد دعاوى متداولة في الفترة")
                ...
                with c1: st.download_button("📄 استخراج PDF", data=to_pdf(pd.DataFrame(df_data), title, region, عضو_قانوني1, lawyer), file_name=f"بيان_الدعاوى_{region}.pdf", mime="application/pdf", use_container_width=True, key="dl1")
                with c2: st.download_button("📝 Word", data=to_word(pd.DataFrame(df_data), title, region, عضو_قانوني1, lawyer), file_name=f"بيان_الدعاوى_{region}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True, key="dl2")
                with c3:
    ...
    with tab2:
        ...
        if st.button("🔍 عرض بيان الاحكام", use_container_width=True, type="primary", key="show2"):
            ...
            if not cases: st.warning(f"⚠️ لا توجد احكام في الفترة")
                ...
                html_export = get_export_html(full_html, st.session_state.last_report_title)
                c1,c2,c3 = st.columns(3)
                with c1: st.download_button("📄 استخراج PDF", data=to_pdf(pd.DataFrame(df_data), title, region2, عضو_قانوني2, lawyer2), file_name=f"بيان_الاحكام_{region2}.pdf", mime="application/pdf", use_container_width=True, key="dl21")
                with c2: st.download_button("📝 Word", data=to_word(pd.DataFrame(df_data), title, region2, عضو_قانوني2, lawyer2), file_name=f"بيان_الاحكام_{region2}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True, key="dl22")
                with c3:
                    excel_buffer = io.BytesIO()
                    with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
                        df = pd.DataFrame(df_data)
    ...
    with tab4:
        ...
        if st.button("🔍 عرض البيان العددي", use_container_width=True, type="primary", key="show_stat"):
            ...
            html_export = get_export_html(full_html, st.session_state.last_report_title)
            c1,c2,c3 = st.columns(3)
            with c1: st.download_button("📄 استخراج PDF", data=to_pdf(df_data, title_stat, region_stat, عضو_قانوني_stat, lawyer_stat), file_name=f"بيان_عددي_{region_stat}.pdf", mime="application/pdf", use_container_width=True, key="dl_stat1")
            with c2: st.download_button("📝 Word", data=to_word(df_data, title_stat, region_stat, عضو_قانوني_stat, lawyer_stat), file_name=f"بيان_عددي_{region_stat}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True, key="dl_stat2")
            with c3:
                excel_buffer = io.BytesIO()
                with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
                    df = df_data
