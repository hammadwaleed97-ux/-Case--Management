...
def messaging_page():
    ...
    attachment_file = st.file_uploader("مرفق إضافي", type=None, key=f"msg_attachment_{sr}", help="PDF أو Word أو Excel أو صورة أو أي ملف آخر")
    ...
    if st.button("📤 إرسال الرسالة / البيان", type="primary", use_container_width=True, key=f"send_message_{sr}"):
        ...
        messages.append({
            ...
            "attachment_data": base64.b64encode(attachment_file.getvalue()).decode("utf-8") if attachment_file else "",
            "attachment_name": attachment_file.name if attachment_file else "",
            "attachment_type": attachment_file.type if attachment_file else "application/octet-stream",
            "in_reply_to": ""
        })
...
def show_top_notices():
    ...
    card += f"<div style='color:{sender_color};font-size:20px;font-weight:900;margin-top:5px;'>المرسل: {html.escape(str(sender_name))}</div>"
    ...
    if item.get("attachment_data"):
        card += f"<div style='margin-top:12px;padding:10px;border:1px solid #D4AF37;border-radius:12px;color:#fff;font-weight:800;'>📎 {html.escape(str(item.get('attachment_name') or 'مرفق'))}</div>"
    card += f"<div style='color:#cbd5e1;font-size:12px;margin-top:10px;'>{created}</div></div>"
    st.markdown(card, unsafe_allow_html=True)
    if item.get("attachment_data"):
        try:
            attachment_bytes = base64.b64decode(item.get("attachment_data"))
            st.download_button(
                "📎 فتح / تنزيل المرفق",
                data=attachment_bytes,
                file_name=item.get("attachment_name") or "مرفق",
                mime=item.get("attachment_type") or "application/octet-stream",
                use_container_width=True,
                key=f"download_notice_attachment_{item.get('id','x')}"
            )
        except Exception:
            pass
    ...
    if n["kind"] == "message":
        ...
        reply_attachment = st.file_uploader("مرفق الرد", type=None, key=f"top_reply_attachment_{item.get('id','x')}", help="PDF أو Word أو Excel أو صورة أو أي ملف")
        if st.button("↩️ إرسال الرد", type="primary", use_container_width=True, key=f"top_reply_send_{item.get('id','x')}"):
            if not reply_text.strip() and not reply_attachment:
                ...
                if target_id is not None:
                    ...
                    msgs.append({
                        ...
                        "image_data": "",
                        "image_type": "",
                        "attachment_data": base64.b64encode(reply_attachment.getvalue()).decode("utf-8") if reply_attachment else "",
                        "attachment_name": reply_attachment.name if reply_attachment else "",
                        "attachment_type": reply_attachment.type if reply_attachment else "application/octet-stream",
