...
def to_pdf(df, title, region="", member_name="", specialization="", manager_name="", general_manager_name="", *args, **kwargs):
    """تصدير PDF عربي RTL مع رأس التقرير وجدول غير متداخل وتوقيعات منظمة."""
    ...
    pdf.cell(0, 7, ar("الهيئة القومية للتأمين الاجتماعي"), 0, 1, "C")
    pdf.cell(0, 6, ar("الإدارة المركزية للإدارات القانونية"), 0, 1, "C")
    pdf.cell(0, 6, ar("الإدارة العامة للقضايا"), 0, 1, "C")
    if clean(region):
        pdf.cell(0, 6, ar(f"ديوان عام منطقة {clean(region)}"), 0, 1, "C")

    pdf.ln(2)
    pdf.set_font(font_name, "", 9.5)
    # عنوان التقرير يُلف داخل عرض الصفحة حتى لا يخرج أي جزء منه خارج الورقة.
    title_text = clean(title)
    if title_text:
        pdf.set_x(12)
        pdf.multi_cell(pdf.w - 24, 6, ar(title_text), 0, "C")
    pdf.ln(2)

    # جدول RTL: كل قيمة تحت عنوانها، مع عرض أكبر للمحكمة والخصوم والموضوع.
    if df is not None and len(getattr(df, "columns", [])):
        ...
        for col in columns:
            ...
            if "المحكمة" in name:
                ...
                weights.append(1.0)
        total = sum(weights) or len(columns)
        widths = [available_w * w / total for w in weights]
        ...
        def wrap_text(text, width):
            ...
            lines, current = [], ""
            for word in words:
                # لو كانت كلمة واحدة أطول من الخانة، نقسمها على أجزاء حتى لا تخرج خارج الجدول.
                if pdf.get_string_width(ar(word)) > width - 3:
                    if current:
                    ...
                    for ch in word:
                        test_chunk = chunk + ch
                        if pdf.get_string_width(ar(test_chunk)) <= width - 3:
                            chunk = test_chunk
                        else:
                            if chunk:
                                lines.append(chunk)
                ...
                if pdf.get_string_width(ar(test)) <= width - 3:
                    current = test
                else:
                    if current:
                        lines.append(current)
        ...
        def draw_row(values, is_header=False):
            ...
            ws = list(reversed(widths))
            line_h = 5.5 if is_header else 5.0
            wrapped = [wrap_text(v, w) for v, w in zip(vals, ws)]
            row_h = min(32, max(line_h, max(len(x) for x in wrapped) * line_h))
            x_right = pdf.w - 10
            y = pdf.get_y()
            for lines, w in zip(wrapped, ws):
                x = x_right - w
    ...
    # العضو يمين ومدير الإدارة يسار في نفس السطر.
    box_w = 100
    margin_x = 18
    right_x = pdf.w - margin_x - box_w
    left_x = margin_x
    y = pdf.get_y()

    pdf.set_xy(right_x, y)
    member_lines = ["عضو الإدارة القانونية"]
    if clean(member_name):
        member_lines.append(clean(member_name))
    member_lines.append(".................................")
    pdf.multi_cell(box_w, 7, ar("\n".join(member_lines)), 0, "C")
    # التاريخ تحت اسم العضو وبعد النقاط مباشرة.
    pdf.set_xy(right_x, pdf.get_y())
    pdf.multi_cell(box_w, 7, ar(f"تحرر في: {datetime.now().strftime('%d/%m/%Y')}"), 0, "C")

    pdf.set_xy(left_x, y)
    manager_lines = ["مدير إدارة القضايا"]
    if clean(manager_name):
        manager_lines.append(clean(manager_name))
    manager_lines.append(".................................")
    pdf.multi_cell(box_w, 7, ar("\n".join(manager_lines)), 0, "C")

    # المدير العام في المنتصف أسفل التوقيعين.
    pdf.set_y(y + 40)
    pdf.cell(0, 7, ar("مدير عام الإدارات القانونية"), 0, 1, "C")
    if clean(general_manager_name):
