...
# ===== مزامنة السحابة =====
SUPABASE_STORAGE_BUCKET = "legal-files"
...
def cloud_success_message(action, section=""):
    target = f" في {section}" if section else ""
    return f"تم {action}{target} والسحابة بنجاح ✓"

def cloud_error_message(action, error):
    return f"تم {action} محليًا، ولكن تعذر إتمام العملية في السحابة: {error}"

def cloud_delete_record(table_name, record_id):
    try:
        if not cloud_status() or not record_id:
            return False, "اتصال السحابة غير متاح"
        supabase.table(table_name).delete().eq("id", record_id).execute()
        return True, ""
    except Exception as exc:
        return False, str(exc)

def cloud_upload_file(file_bytes, file_name, content_type="application/octet-stream"):
    try:
        if not cloud_status():
            return None, "اتصال السحابة غير متاح"
        safe_name = re.sub(r"[^A-Za-z0-9._/-]+", "_", str(file_name))
        storage_path = f"{datetime.now().strftime('%Y/%m')}/{safe_name}"
...
def cloud_delete_file(storage_path):
    try:
        if not cloud_status() or not storage_path:
            return False, "مسار الملف السحابي غير موجود"
...
def _cloud_put_db(key, payload):
    ...
    try:
        ...
        if r.ok:
            return True
        _CLOUD_LAST_ERROR = f"Database HTTP {r.status_code}: {r.text[:500]}"
    except Exception as exc:
        _CLOUD_LAST_ERROR = f"Database: {exc}"
    return False

def _cloud_get_db(key):
    global _CLOUD_LAST_ERROR
    try:
        r = requests.get(
            f"{SUPABASE_URL}/rest/v1/app_state",
            params={"key": f"eq.{key}", "select": "key,payload,updated_at"},
            headers=_cloud_headers(),
            timeout=12,
        )
        if r.ok:
...
def _storage_headers(content_type="application/json"):
    return {
        "apikey": SUPABASE_KEY,
        "Authorization": f"Bearer {SUPABASE_KEY}",
...
def _cloud_put_storage(key, payload):
    global _CLOUD_LAST_ERROR
    try:
        storage_key = f"app_state/{key}.json"
        ...
        if r.ok:
            return True
        _CLOUD_LAST_ERROR = f"Storage HTTP {r.status_code}: {r.text[:500]}"
    except Exception as exc:
        _CLOUD_LAST_ERROR = f"Storage: {exc}"
    return False

def _cloud_get_storage(key):
    global _CLOUD_LAST_ERROR
    try:
        storage_key = f"app_state/{key}.json"
        r = requests.get(
...
def _cloud_put(key, payload):
    ...
    if not CLOUD_ENABLED:
        _CLOUD_LAST_ERROR = "SUPABASE_URL أو SUPABASE_KEY غير موجود في Secrets."
        return False
    if _cloud_put_db(key, payload):
        return True
    return _cloud_put_storage(key, payload)
...
def _cloud_get(key):
    ...
    if not CLOUD_ENABLED:
        _CLOUD_LAST_ERROR = "SUPABASE_URL أو SUPABASE_KEY غير موجود في Secrets."
        return None
    row = _cloud_get_db(key)
    if row is not None:
        return row
    return _cloud_get_storage(key)
...
def save_json_persistent(key, file_path, data):
    """
    حفظ متوافق مع جميع الاستدعاءات القديمة:
    save_json_persistent(key, file_path, data)
    السحابة أولاً، والمحلي نسخة احتياطية فقط.
    """
    try:
        cloud_ok = bool(_cloud_put(key, data))
    except Exception as exc:
        st.error(f"❌ لم يتم الحفظ في السحابة: {exc}")
        return False

    if not cloud_ok:
        st.error(f'❌ لم يتم الحفظ في السحابة: {_CLOUD_LAST_ERROR or "Supabase لم يؤكد العملية."}')
        return False

    # Cache محلي بعد نجاح السحابة.
    try:
        path = Path(file_path)
...
def to_pdf(df, title, region, member_name="", specialization="", manager_name="", general_manager=""):
    pdf=FPDF(orientation="L", unit="mm", format="A4"); pdf.add_page()
    fp=_report_font_path(); font="ArabicReport" if fp else "Helvetica"
    if fp: pdf.add_font(font,"",fp,uni=True)
    pdf.set_font(font,"",13)
    def ar(v):
        s="" if v is None else str(v)
        try: return fix_arabic(s)
        except Exception: return s
    pdf.cell(0,9,ar("الهيئة القومية للتأمين الاجتماعي"),0,1,"C")
    pdf.set_font(font,"",11); pdf.cell(0,7,ar("الإدارة المركزية للإدارات القانونية"),0,1,"C")
    pdf.cell(0,7,ar("الإدارة العامة للقضايا"),0,1,"C")
    if region: pdf.cell(0,7,ar(f"ديوان عام منطقة {region}"),0,1,"C")
    pdf.cell(0,8,ar(title),0,1,"C"); pdf.ln(3)
    if len(df.columns):
        pdf.set_font(font,"",7); width=280/len(df.columns)
        for col in df.columns: pdf.cell(width,7,ar(col),1,0,"C")
        pdf.ln()
        for _,row in df.iterrows():
            for val in row: pdf.cell(width,7,ar(val),1,0,"C")
            pdf.ln()
    pdf.ln(6); pdf.set_font(font,"",9)
    pdf.cell(0,7,ar(f"تحريرًا في: {datetime.now().strftime('%d-%m-%Y')}"),0,1,"R")
    pdf.ln(5); pdf.cell(0,7,ar("وتفضلوا بقبول وافر الاحترام والتقدير،"),0,1,"R")
    pdf.ln(8); pdf.cell(0,7,ar("عضو الإدارة القانونية"),0,1,"R"); pdf.cell(0,7,ar(member_name),0,1,"R"); pdf.cell(0,7,ar("...................."),0,1,"R")
    pdf.ln(5); pdf.cell(0,7,ar("مدير إدارة القضايا"),0,1,"R"); pdf.cell(0,7,ar(manager_name),0,1,"R"); pdf.cell(0,7,ar("...................."),0,1,"R")
    pdf.ln(5); pdf.cell(0,7,ar("مدير عام الإدارات القانونية"),0,1,"R"); pdf.cell(0,7,ar(general_manager),0,1,"R"); pdf.cell(0,7,ar("...................."),0,1,"R")
    raw=pdf.output(); return raw.encode("latin-1",errors="replace") if isinstance(raw,str) else bytes(raw)
