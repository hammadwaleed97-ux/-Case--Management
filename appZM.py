...
def extract_member_page():
    ...
    with st.container(border=True):
        ...
        job_role = st.selectbox("الوظيفة", ["عضو قانونى", "مدير إدارة", "مدير عام (قيادى مناطق)", "قيادي مركز رئيسى 1", "قيادي مركز رئيسى 2", "قيادي مركز رئيسى 3"], key="new_member_job_role")
        ...
        if st.button("🪪 استخراج العضوية", use_container_width=True, type="primary", key="extract_member_new_v3"):
            ...
            if not name:
                ...
                if existing_user:
                    ...
                    existing_user["role"] = {"عضو قانونى":"member", "مدير إدارة":"manager", "مدير عام (قيادى مناطق)":"general_manager", "قيادي مركز رئيسى 1":"leader1", "قيادي مركز رئيسى 2":"leader2", "قيادي مركز رئيسى 3":"leader3"}.get(job_role, "member")
                    ...
                    users.append({
                        ...
                        "email": "",
                        "recovery_email": "",
                        "role": {"عضو قانونى":"member", "مدير إدارة":"manager", "مدير عام (قيادى مناطق)":"general_manager", "قيادي مركز رئيسى 1":"leader1", "قيادي مركز رئيسى 2":"leader2", "قيادي مركز رئيسى 3":"leader3"}.get(job_role, "member"),
                        "job_title": job_role,
                        "status": "active",
                        "password_set": False,
...
def messaging_page():
    ...
    target_options = [
        ...
        ("مدير عام (قيادى مناطق)", "general_manager"), ("قيادي مركز رئيسى 1", "leader1"),
    ...
    for area in sorted(set((u.get("insurance_area") or "غير محددة").strip() or "غير محددة" for u in users), key=lambda x:x.casefold()):
        ...
        for i,u in enumerate(area_users):
            with cols[i % 6]:
                ...
                job_value = "مدير عام (قيادى مناطق)" if (u.get("role") == "general_manager" or raw_job in ("مدير عام (قيادى مناطق)", "مدير عام (قيادى مناطق)", "مدير عام (قيادى مناطق)")) else (raw_job or role_label(u.get("role")))
...
def admin_control_page():
    ...
    with tab_search:
        st.markdown("<h3 style='color:#D4AF37;'>🔎 البحث عن عضو أو مدير إدارة أو مدير عام (قيادى مناطق)أو قيادي</h3>", unsafe_allow_html=True)
...
def to_pdf(df, title, region):
    ...
    pdf.cell(cell_w, 8, fix_arabic('مدير عام (قيادى مناطق)الإدارات القانونية'), 0, 1, 'C')
...
if st.session_state.page == "تقارير":
  ...
  .stButton>button:hover {background: #B8860B!important; color: #000!important;}
    ...
    with tab1:
        ...
        with colB: مدير_عام1 = st.text_input("اسم مدير عام (قيادى مناطق)الإدارات القانونية", key="modir1")
    ...
    with tab2:
        ...
        with colB: مدير_عام2 = st.text_input("اسم مدير عام (قيادى مناطق)الإدارات القانونية", key="modir2")
    ...
    with tab4:
        ...
        with colB: مدير_عام_stat = st.text_input("اسم مدير عام (قيادى مناطق)الإدارات القانونية", key="modir_stat")
