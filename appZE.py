...
button svg, [data-testid="stButton"] svg, [data-testid="stDownloadButton"] svg {
...
def login_page():
    ...
    with tab2:
        ...
        if st.button("تفعيل الحساب", use_container_width=True, key="activate_account"):
            ...
            found_user = next((
                u for u in users
                if str(u.get("username", "")).strip().casefold() == search_name
                or str(u.get("display_name", "")).strip().casefold() == search_name
            ), None)
            if not found_user:
                st.error("الاسم ده مش موجود في العضويات المستخرجة. اكتب الاسم كما هو مسجل عند استخراج العضوية.")
                ...
                st.session_state.page = "set_password"
                st.session_state.temp_user_id = found_user.get("id")
                st.session_state.temp_user = found_user.get("username")
                st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
...
# التسلسل الوظيفي للرسائل. الادمن يظل صاحب الصلاحية الكاملة دائمًا.
def can_message(sender_role, target_role):
    ...
    if sender_role == "member":
        return target_role in ("member", "manager")
    if sender_role == "manager":
        # مدير الإدارة: إرسال للادمن، والمدير العام، ومديري الإدارات، والأعضاء.
...
def person_display(u, include_role=True):
    """اسم ظاهر + الوظيفة + المنطقة التأمينية للمستلمين، مع إبقاء صفة الادمن واضحة."""
...
def messaging_page():
    ...
    # عرض المستلمين حسب صلاحيات الوظيفة.
    target_options = [
    ...
    target_kind = st.selectbox(
        "إرسال إلى",
        ["الجميع"] + [label for label, role in target_options if role in allowed_roles] + ["محددون من أكثر من وظيفة"],
        key=f"msg_target_kind_{sr}"
    )
    text = st.text_area("نص الرسالة أو البيان", height=130, key=f"msg_text_{sr}")

    role_by_label = dict(target_options)
    selected = []
    if target_kind == "محددون من أكثر من وظيفة":
        candidates = [u for u in users if u.get("status") == "active" and can_message(sr, u.get("role"))]
        selected = st.multiselect(
            "اختر المستلمين",
            [person_display(u) for u in candidates],
            key=f"msg_multi_{sr}"
        )
    elif target_kind != "الجميع":
        rr = role_by_label.get(target_kind)
        candidates = [u for u in users if u.get("status") == "active" and u.get("role") == rr and can_message(sr, rr)]
        selected = st.multiselect(
            f"اختر {target_kind}",
            [person_display(u) for u in candidates],
            key=f"msg_selected_{sr}_{rr}"
        )
    ...
    if st.button("📤 إرسال الرسالة / البيان", type="primary", use_container_width=True, key=f"send_message_{sr}"):
        ...
        recipients = []
        selected_names = {x.split(" — ", 1)[0] for x in selected}
        for u in users:
            ...
            if target_kind == "الجميع":
                ok = True
            elif target_kind == "محددون من أكثر من وظيفة":
                ok = u.get("username") in selected_names
            else:
                ok = u.get("username") in selected
            if ok and u.get("id") != user.get("id"):
...
def show_messages_for_user():
    """عرض الرسائل الواردة أعلى الصفحة، رسالة واحدة في كل مرة مع تنقل."""
    user = st.session_state.get("user") or {}
    uid = user.get("id")
    if not uid:
        return

    msgs = [m for m in load_messages() if uid in m.get("recipient_ids", [])]
    if not msgs:
        return

    msgs = list(reversed(msgs[-50:]))
    total = len(msgs)
    idx = int(st.session_state.get("incoming_message_index", 0))
    idx = max(0, min(idx, total - 1))
    st.session_state.incoming_message_index = idx
    m = msgs[idx]
    ...
    st.markdown(
        f'<div style="background:linear-gradient(135deg,#14233d,#203b63);border:2px solid #D4AF37;border-radius:16px;padding:18px;margin:0 0 10px;box-shadow:0 8px 24px rgba(0,0,0,.25);">'
        f'<div style="color:#D4AF37;font-size:20px;font-weight:900;">📢 المرسل: {sender}</div>'
        f'<div style="color:#fff;font-size:16px;font-weight:800;margin-top:5px;">الوظيفة: {sender_job} — الصفة: {role}</div>'
        f'<div style="color:#fff;font-size:18px;font-weight:700;margin-top:12px;white-space:pre-wrap;">{message_text}</div>'
        f'<div style="color:#cbd5e1;font-size:12px;margin-top:10px;">{created}</div></div>', unsafe_allow_html=True)

    c1,c2,c3 = st.columns([1,1,1])
    with c1:
        if st.button("⬅️ السابقة", disabled=(idx >= total-1), use_container_width=True, key="incoming_prev"):
            st.session_state.incoming_message_index = min(total-1, idx+1)
            st.rerun()
    with c2:
    ...
    with c3:
        if st.button("التالية ➡️", disabled=(idx <= 0), use_container_width=True, key="incoming_next"):
            st.session_state.incoming_message_index = max(0, idx-1)
            st.rerun()
...
def set_password_page():
    """صفحة إنشاء كلمة السر لأول دخول للعضوية المستخرجة."""
    ...
    if st.button("حفظ كلمة السر والدخول", type="primary", use_container_width=True, key="save_first_password"):
        ...
        for user in users:
            if target_id is not None and user.get("id") == target_id:
                target = user
                break
            if str(user.get("username", "")).strip().casefold() == target_name or str(user.get("display_name", "")).strip().casefold() == target_name:
                target = user
                break
        if not target:
...
def personal_control_page():
    """لوحة تحكم شخصية لكل مستخدم غير الادمن."""
    user = st.session_state.get("user") or {}
    role = user.get("role")
...
# تعريف مبكر لأن الصفحة الرئيسية تستخدمه قبل تعريفات الصفحات المتأخرة.
...
# لا تظهر أقسام الموقع إلا بعد تسجيل الدخول.
if not st.session_state.get("user"):
    st.session_state.page = "login"
    login_page()
    st.stop()
...
if st.session_state.page == "الرئيسية":
    ...
    if is_admin:
        ...
        if st.button("⚙️ لوحة التحكم", use_container_width=True, type="primary", key="open_admin_control_top_v2"):
            st.session_state.page = "لوحة التحكم"
            st.rerun()
    else:
        ...
        if st.button("👤 فتح لوحة تحكمى", use_container_width=True, type="primary", key="open_personal_control_top_v3"):
            st.session_state.page = "لوحة تحكمى"
            st.rerun()

    # البحث عن الأشخاص أصبح داخل لوحة التحكم فقط للادمن.
    ...
    if user.get("role") in ("admin", "member", "manager", "general_manager", "leader1", "leader2", "leader3"):
        a,b,c = st.columns([1,1.2,1])
        with b: judicial_card("📢","الرسائل والبيانات","الرسائل","card_messages_v2")

    a,b = st.columns(2)
    with a: judicial_card("🗄️","الأرشيف","الأرشيف","card_archive_v2")
    with b:
        if is_admin:
            judicial_card("⚙️","لوحة التحكم","لوحة التحكم","card_admin_panel_bottom_v3")
        else:
            judicial_card("👤","لوحة تحكمى","لوحة تحكمى","card_personal_control_v3")
