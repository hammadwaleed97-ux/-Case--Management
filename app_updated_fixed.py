...
def _age_action_dialog(user_id):
    ...
    def age_dialog():
        ...
        if action == "إيقاف عضوية لبلوغ السن":
            ...
            targets = [u for u in users if u.get("role") == "member" and u.get("id") != user.get("id") and u.get("status") == "active"]
            if not targets:
...
def admin_control_page():
    ...
    tab_users, tab_banners, tab_account = st.tabs(["👥 الأعضاء", "📢 اليافطات", "🔐 حساب الادمن"])
    ...
    with tab_banners:
        st.markdown("<h2 style='color:#D4AF37;text-align:center;'>📢 إدارة اليافطات</h2>", unsafe_allow_html=True)
        users = load_users()
        ...
        with st.form("admin_banner_add_v2"):
            text = st.text_area("نص اليافطة", height=90)
            color = st.color_picker("لون اليافطة", "#D4AF37")
            ...
            audience = st.radio("تظهر لـ", ["كل الأعضاء","أعضاء محددين"], horizontal=True, key="banner_audience_v2")
            visible_to = []
            if audience == "أعضاء محددين":
                visible_to = st.multiselect("حدد الأعضاء", [u.get("username") for u in users if u.get("role") == "member"], key="banner_users_v2")
            if st.form_submit_button("🚀 نشر اليافطة", use_container_width=True, type="primary"):
                if not text.strip():
                    st.error("اكتب نص اليافطة")
                    ...
                    st.success("تم نشر اليافطة")
                    st.rerun()

        st.markdown("---")
        st.markdown(f"<h3 style='color:#D4AF37;'>اليافطات الحالية: {len(banners)}</h3>", unsafe_allow_html=True)
            ...
            with st.container(border=True):
                ...
                if st.session_state.get("edit_banner_id") == b.get("id"):
                    with st.form(f"edit_banner_form_{b.get('id',i)}"):
                        nt = st.text_area("نص اليافطة", value=b.get("text",""), key=f"et_{b.get('id',i)}")
                        nc = st.color_picker("اللون", value=b.get("color","#D4AF37"), key=f"ec_{b.get('id',i)}")
                        na = st.radio("الجمهور", ["كل الأعضاء","أعضاء محددين"], index=0 if b.get("audience") in ("الكل","كل الأعضاء") else 1, horizontal=True, key=f"ea_{b.get('id',i)}")
                        nvis = []
                        if na == "أعضاء محددين":
                            nvis = st.multiselect("الأعضاء", [u.get("username") for u in users if u.get("role")=="member"], default=b.get("visible_to",[]), key=f"ev_{b.get('id',i)}")
...
if st.session_state.get("page") == "لوحة التحكم":
    if st.session_state.get("user", {}).get("role") == "admin":
        ...
        st.session_state.page = "login"
        st.rerun()
elif st.session_state.get("page") == "إدارة اليافطات":
    if st.session_state.get("user", {}).get("role") == "admin":
        admin_control_page()
    else:
        st.session_state.page = "login"
