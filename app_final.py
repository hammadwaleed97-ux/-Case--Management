...
# إعدادات البريد يجب أن تكون معرفة قبل أي دالة تستخدمها
try:
...
def load_users():
    if supabase is None:
        ...
        return [{"id": 1, "username": ADMIN_USERNAME, "password": admin_pass, "role": "admin", "status": "active", "password_set": True, "email": "", "recovery_email": ""}]
    try:
        res = supabase.table("users").select("*").execute()
        users = res.data or []
        if not any(u.get("username") == ADMIN_USERNAME for u in users):
            admin_pass = bcrypt.hashpw(ADMIN_DEFAULT_PASS.encode(), bcrypt.gensalt()).decode()
            supabase.table("users").insert({"username": ADMIN_USERNAME, "password": admin_pass, "role": "admin", "status": "active", "password_set": True, "email": ""}).execute()
            return load_users()
        return users
    except Exception as e:
        st.warning(f"تعذر الاتصال بالسحابة: {e}. سيتم استخدام التخزين المحلي.")
        admin_pass = bcrypt.hashpw(ADMIN_DEFAULT_PASS.encode(), bcrypt.gensalt()).decode()
        return [{"id": 1, "username": ADMIN_USERNAME, "password": admin_pass, "role": "admin", "status": "active", "password_set": True, "email": "", "recovery_email": ""}]
...
def set_password_page():
    st.markdown("<h2 style='text-align:center; color:#C9A961'>تعيين كلمة سر جديدة</h2>", unsafe_allow_html=True)
    username = st.session_state.get("temp_user", "")
    if not username:
        st.session_state.page = "login"
        st.rerun()
    new_pass = st.text_input("كلمة السر الجديدة", type="password", key="set_new_pass")
    confirm_pass = st.text_input("تأكيد كلمة السر", type="password", key="set_confirm_pass")
    if st.button("حفظ كلمة السر", use_container_width=True, type="primary"):
        if not new_pass:
            st.error("اكتب كلمة السر")
        elif new_pass != confirm_pass:
            st.error("كلمتا السر غير متطابقتين")
        elif len(new_pass) < 6:
            st.error("كلمة السر يجب ألا تقل عن 6 أحرف")
        else:
            users = load_users()
            ...
            if not user:
                ...
                hashed = bcrypt.hashpw(new_pass.encode(), bcrypt.gensalt()).decode()
                update_user_db(user["id"], {"password": hashed, "password_set": True, "status": "active"})
                logged_user = check_login(username, new_pass)
                if logged_user:
                    st.session_state.user = logged_user
                    st.session_state.role = logged_user["role"]
                    ...
                    st.error("تعذر تسجيل الدخول بعد تفعيل الحساب")
...
def to_pdf(df, title, region):
    ...
    return bytes(pdf.output()) # <--- التعديل عشان يرجع bytes
...
def load_data():
    # 1. نجيب من السحابة أولاً، وإن لم تكن مفعلة نستخدم المحلي
    if supabase is None:
...
def save_data(data):
    ...
    # 2. نحفظ في السحابة إذا كانت مفعلة
    if supabase is None:
        return
    try:
...
# ====== الجزء الثاني: تسجيل القضية ============
    ...
    نوع = st.selectbox("اختيار", ["دعوى", "استئناف", "طعن"], key="case_type_add")
    ...
    if st.session_state.get('confirm_delete', False):
        ...
        with col1:
            if st.button("نعم احذفها", use_container_width=True, type="primary"):
                ...
                supabase.table("cases").delete().eq("id", case["id"]).execute() if supabase else None
            ...
            if st.session_state.get('del_arch_id') == case['id']:
                ...
                with c1:
                    if st.button("نعم احذف", key=f"confirm_del_{case['id']}"):
                        supabase.table("cases").delete().eq("id", case["id"]).execute() if supabase else None # <--- حذف من السحابة
                        data["cases"] = [c for c in data["cases"] if c["id"]!= case["id"]]
