...
def messaging_page():
    ...
    attachment_file = st.file_uploader("📎 إرسال المرفق", type=None, key=f"msg_attachment_{sr}", help="PDF أو Word أو Excel أو صورة أو أي ملف آخر")
    image_file = st.file_uploader("🖼️ إرسال الصورة", type=["jpg","jpeg","png"], key=f"msg_image_{sr}")
    ...
    for area in sorted(set((u.get("insurance_area") or "غير محددة").strip() or "غير محددة" for u in users), key=lambda x:x.casefold()):
        ...
        for i,u in enumerate(area_users):
            with cols[i % 6]:
                ...
                job = html.escape(str(("مدير عام (قيادى مناطق)" if u.get("role") == "general_manager" else (u.get("job_title") or role_label(u.get("role"))))) )
